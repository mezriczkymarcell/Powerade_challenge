...
function PhotoThumb({ url, size = 52, className = "" }) {
  ...
  // A megnyitott kép a lap tetejére kerül (portál), így semmi nem tud fölé csúszni.
  useEffect(() => {
    if (!open) return;
  ...
  const lightbox = (
    <div className="fixed inset-0 flex items-center justify-center p-4"
      style={{ zIndex: 2147483000, background: "rgba(9,13,20,.88)", animation: "pcFade 180ms ease-out" }}
      ...
      <img src={url} alt="igazolás"
        onClick={(e) => { e.stopPropagation(); setOpen(false); }}
        className="max-h-[86vh] max-w-full cursor-zoom-out rounded-2xl object-contain shadow-2xl"
  ...
  return (
    <>
      ...
      {open && typeof document !== "undefined" && createPortal(lightbox, document.body)}
    </>
  );
}
