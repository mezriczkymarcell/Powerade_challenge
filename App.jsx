    ...
    <div className="space-y-5">
      ...
      <Card className="overflow-hidden">
        <div className="flex items-center gap-3 px-4 py-3 text-xs font-bold uppercase tracking-wider"
          ...
          <span className="w-24 text-right">Hiányzik</span>
        </div>
        {rows.map((r, i) => (
          <button key={r.id} onClick={() => onOpenMember(r.id)}
            ...
            <span className="w-24 shrink-0 self-start pt-0.5 text-right">
              {r.done ? (
                <span className="text-sm font-black" style={{ color: C.ok }}>kész ✓</span>
              ) : (
                <>
                  <span className="block text-sm font-black tabular-nums" style={{ color: C.blue }}>
                    {r.left.toLocaleString("hu-HU")}
                  </span>
                  {tab === "week" && (
                    <span className="block text-xs font-bold tabular-nums" style={{ color: C.slate }}>
                      {r.perDay.toLocaleString("hu-HU")} / nap
                    </span>
                  )}
                </>
              )}
      ...
      {tab === "week" && (
        <p className="px-1 text-xs" style={{ color: C.slate }}>
          A „Hiányzik" oszlop a heti célból hátralévő kalóriát mutatja, alatta pedig, hogy a maradék{" "}
          <b>{daysLeft} napra</b> elosztva mennyit kell naponta összeszedni.
        </p>
      )}
