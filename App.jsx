...
function FeedItem({ item, likes, meId, onLike }) {
  ...
  return (
    <div className="flex gap-3 py-3.5" style={{ borderTop: `1px solid ${C.line}` }}>
      ...
      {!(item.type === "weight" && u.hide_weight && u.id !== meId) && (
...
/* ================================ SÚLYMODUL ============================== */
function WeightChart({ members, meId }) {
        ...
        {series.map((s, i) => {
          ...
          return (
            <g key={s.user.id}>
              ...
              {/* címke a végén */}
              {(() => {
                const hidden = s.user.hide_weight && s.user.id !== meId;
    ...
    <div className="space-y-5">
      <Card className="p-6">
        ...
        {withWeights.length === 0 ? (
          ...
          <>
            <WeightChart members={withWeights} meId={me.id} />
            <p className="mt-2 text-center text-xs" style={{ color: C.slate }}>
      ...
      {mine.length > 0 && (
        <Card className="p-6">
          <SectionTitle right={me.hide_weight && <Pill tone="slate">rejtve</Pill>}>Az én mérésem</SectionTitle>
      ...
      {isOwner && (
        <Card className="p-6">
          ...
          <div className="space-y-4">
            ...
            <div>
              <Field label="A kihívás kezdete" type="date" value={gStart}
                onChange={(e) => e.target.value && setGStart(e.target.value)} />
              <div className="mt-2 rounded-xl p-3" style={{ background: `${C.amber}0D` }}>
                <p className="text-xs font-bold" style={{ color: C.ink }}>
                  Hétfőre igazítva: {fmtLong(mondayOf(gStart))}
                </p>
                <p className="text-xs" style={{ color: C.slate }}>
                  Vége: {fmtLong(addDays(mondayOf(gStart), TOTAL_DAYS - 1))}
                </p>
                {mondayOf(gStart) !== group.start_date && (
                  <p className="mt-1.5 flex items-start gap-1.5 text-xs font-semibold" style={{ color: C.amber }}>
                    <AlertTriangle size={12} className="mt-0.5 shrink-0" />
                    A dátum módosításával a hetek és a büntetések újraszámolódnak. A már rögzített
                    aktivitások megmaradnak, de átkerülnek a megfelelő hetekbe.
                  </p>
                )}
              </div>
            </div>
