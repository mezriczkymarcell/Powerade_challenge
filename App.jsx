...
import {
  ...
  Crown, Settings, User, Calendar, Target, Zap, Award, TrendingUp, Ban, DoorOpen, MessageSquare
} from "lucide-react";
...
function evaluate(user, group) {
  ...
  const weeks = Array.from({ length: TOTAL_WEEKS }, (_, w) => evaluateWeek(user.entries, start, w, goal, penaltyOf(group)));
...
/** Profilkép: emoji + a név monogramja, hogy egy pillantásra felismerhető legyen. */
const Avatar = ({ user, size = 40, badge = true }) => (
  <span className="relative inline-flex shrink-0 items-center justify-center rounded-xl"
    style={{ width: size, height: size, background: C.soft, fontSize: size * 0.5 }}
    title={user?.name}>
    {user?.emoji}
    {badge && (
      <span className="absolute -bottom-1 -right-1 flex items-center justify-center rounded-md px-1 font-black"
        style={{
          background: C.ink, color: "#fff", fontSize: Math.max(8, size * 0.24),
          lineHeight: 1.5, border: "1.5px solid #fff", letterSpacing: "0.02em",
        }}>
        {initials(user?.name)}
      </span>
    )}
          ...
          <Card className="mb-4 space-y-4 p-5">
            ...
            <div>
              ...
              <p className="mt-1 text-xs" style={{ color: C.slate }}>
                Később is bármikor hívhatsz be embereket a csoport oldaláról.
              </p>
            </div>
...
const ActivityRow = ({ it, onDelete }) => (
  <div className="flex items-center gap-3 rounded-xl px-3 py-3" style={{ border: `1px solid ${C.line}` }}>
    ...
    <div className="min-w-0 flex-1">
      ...
      {it.note && (
        <p className="flex items-center gap-1 truncate text-xs" style={{ color: C.slate }}>
          <MessageSquare size={10} className="shrink-0" /> {it.note}
        </p>
      )}
      {it.proof_url && (
        <a href={it.proof_url} target="_blank" rel="noreferrer" onClick={(e) => e.stopPropagation()}
          className="inline-flex items-center gap-1 text-xs font-semibold" style={{ color: C.slate }}>
          <Camera size={11} /> igazolás
        </a>
      )}
    </div>
...
function TrackerView({ me, group, sports, onAddSport, onSave, onDelete, toast, initialDate }) {
  ...
  const clearForm = () => {
    setKcal(""); setNote(""); setFile(null); setPreview(null);
  ...
  const save = async () => {
    ...
    setBusy(true);
    const ok = await onSave(date, n, sport, note.trim() || null, file);
    setBusy(false);
        ...
        <div className="space-y-4">
          ...
          <div>
            ...
            <input value={note} onChange={(e) => setNote(e.target.value)} maxLength={40}
              placeholder="pl. Hajnali kör a Duna-parton"
              onKeyDown={(e) => e.key === "Enter" && save()}
              className="w-full rounded-xl px-4 py-3 text-base outline-none"
              style={{ background: C.soft, border: `1px solid ${C.line}`, color: C.ink }} />
            <p className="mt-1 text-xs" style={{ color: C.slate }}>Néhány szó, ha van kedved. {40 - note.length} karakter maradt.</p>
          </div>
          ...
          {weekDays(group.start_date, cw).map((d) => {
            ...
            return (
              <button key={d} onClick={() => !future && setDate(d)} disabled={future}
                ...
                {e?.items.some((x) => x.proof_url) && <Camera size={14} style={{ color: C.slate }} />}
...
function HighlightStats({ group }) {
  const stats = useMemo(() => {
    ...
    const sportCount = {};
    rows.forEach(({ u }) => Object.entries(u.entries).forEach(([d, e]) => {
      if (d >= start && d <= end) e.items.forEach((it) => {
        sportCount[it.sport] = (sportCount[it.sport] || 0) + 1;
      });
    }));
...
function MemberView({ user, group, meId, onBack }) {
  ...
  // Napok visszafelé, csak ahol volt aktivitás
  ...
  return (
    <div className="space-y-5">
      <button onClick={onBack} className="inline-flex items-center gap-1 text-sm font-bold" style={{ color: C.blue }}>
        <ArrowLeft size={16} /> Vissza a ranglistához
      </button>
...
function LeaderboardView({ group, meId, onOpenMember }) {
  ...
  const rows = useMemo(() => group.members.map((u) => {
    ...
    return {
      id: u.id, name: u.name, emoji: u.emoji, goal: s.goal, target, kcal,
      pct: Math.min(kcal / target, 1),
      done: kcal >= target, paid: s.paid, debt: s.debt + s.pending,
      ...
      <Card className="overflow-hidden">
        ...
        {rows.map((r, i) => (
          <button key={r.id} onClick={() => onOpenMember(r.id)}
            className="flex w-full items-center gap-3 px-4 py-3.5 text-left transition hover:opacity-80"
            style={{ borderTop: `1px solid ${C.line}`, background: r.id === meId ? `${C.cyan}0F` : "#fff" }}>
            <span className="w-6 shrink-0 text-base font-black tabular-nums" style={{ color: i === 0 ? C.blue : C.slate }}>{i + 1}</span>
            ...
            <div className="min-w-0 flex-1">
              ...
              {/* Haladás a saját céljához képest */}
              ...
              <div className="mt-1.5 flex flex-wrap items-center gap-2">
                {r.done ? <Pill tone="ok"><Check size={11} /> Megvan</Pill>
                  : <Pill tone="blue">cél: {r.target.toLocaleString("hu-HU")}</Pill>}
            ...
            <span className="w-20 shrink-0 self-start pt-0.5 text-right text-sm font-black tabular-nums" style={{ color: C.ink }}>
              {r.kcal.toLocaleString("hu-HU")}
            </span>
            {pen && (
              <span className="w-24 shrink-0 self-start pt-0.5 text-right text-sm font-bold tabular-nums"
                style={{ color: r.paid ? C.ok : C.slate }}>
                {money(r.paid)}
              </span>
            )}
            <ChevronRight size={15} className="shrink-0 self-start" style={{ color: C.line, marginTop: 2 }} />
          </button>
        ))}
      </Card>
...
/* ============================== Kassza / Tagok ============================ */
function WalletView({ me, group, stats, pool, onDeclare, onApprove, onInvite, onLeave, onUpdateGroup, onOpenMember, toast }) {
              ...
              {gPen && (
                <div className="mt-3">
                  ...
                  <input type="number" inputMode="numeric" value={gAmount} onChange={(e) => setGAmount(e.target.value)}
                    ...
                    style={{ background: "#fff", border: `1px solid ${C.line}`, color: C.ink }} />
                  <p className="mt-1 text-xs" style={{ color: C.slate }}>
                    A módosítás a korábbi heteket is átszámolja.
                  </p>
                </div>
              )}
            </div>
          ...
          <div className="space-y-2">
            {failed.map((w) => (
              <div key={w.weekIndex} className="flex items-center gap-3 rounded-xl px-3 py-3" style={{ border: `1px solid ${C.line}` }}>
                ...
                <span className="text-sm font-black tabular-nums" style={{ color: C.danger }}>{money(w.penalty)}</span>
              </div>
            ))}
          </div>
        </Card>
...
export default function App() {
  ...
  const load = useCallback(async () => {
    ...
    const users = (pr.data || []).map((p) => {
      ...
      Object.values(byDate).forEach((d) => d.items.sort((a, b) => (a.created_at || "").localeCompare(b.created_at || "")));
      return { ...p, entries: byDate, payments: (pa.data || []).filter((x) => x.user_id === p.id) };
    });
    setProfiles(users);
  ...
  const createGroup = async (g, emails = []) => {
    ...
    if (emails.length) {
      const { error: e3 } = await supabase.from("invites")
        .insert(emails.map((email) => ({ group_id: data.id, email })));
      if (e3) notify("A csoport elkészült, de a meghívók nem mentek ki: " + e3.message, "danger");
    }
    await load();
    return true;
  };
  ...
  const acceptInvite = async (inv) => {
    // Visszatérő tag: a régi sorát élesztjük újra, hogy minden adata megmaradjon
    const { data: back } = await supabase.from("group_members")
      .update({ left_at: null })
  ...
  const addEntry = async (date, kcal, sport, note, file) => {
    let proof_url = null;
    if (file) {
    ...
    const { error } = await supabase.from("entries")
      .insert({ user_id: session.user.id, date, kcal, sport, note, proof_url });
    ...
    <div className="min-h-screen pb-24 md:pb-8" style={{ background: "#fff" }}>
      <header className="sticky top-0 z-20 backdrop-blur"
        ...
        <div className="mx-auto flex max-w-3xl items-center gap-2 px-5 py-3">
          <button onClick={() => { setOpenId(null); setMemberId(null); setScreen("groups"); }} className="rounded-lg p-2">
          ...
          <div className="hidden gap-1 md:flex">
            {NAV.map(([k, l, Icon]) => (
              <button key={k} onClick={() => { setTab(k); setMemberId(null); }}
        ...
        {member ? (
          ...
          <>
            ...
            {tab === "wallet" && <WalletView me={me} group={group} stats={stats} pool={pool}
              onDeclare={declare} onApprove={approve} onInvite={inviteEmail} onOpenMember={setMemberId}
      ...
      <nav className="fixed bottom-0 left-0 right-0 z-20 md:hidden"
        ...
        <div className="mx-auto flex max-w-3xl">
          {NAV.map(([k, l, Icon]) => {
            const on = tab === k;
            return (
              <button key={k} onClick={() => { setTab(k); setMemberId(null); }} className="flex flex-1 flex-col items-center gap-1 py-3">
