...
function MotivArt({ i }) {
  ...
  const A = {
    ...
    /* 11 — pihenő figura, fölszálló és elpukkanó kifogás-buborékok */
    11: wrap(null,
      <>
        {shadow(150, 184, 70)}
        {/* fekvő figura */}
        <ellipse cx="150" cy="164" rx="52" ry="16" fill="url(#skin)" />
        <circle cx="106" cy="150" r="15" fill="url(#skin)" />
        <path d="M92 146 a15 10 0 0 1 28 0z" fill="#4E342E" />
        <line x1="98" y1="152" x2="108" y2="152" stroke="#2B2118" strokeWidth="1.8" strokeLinecap="round" />
        <ellipse cx="164" cy="158" rx="22" ry="11" fill="#F0B98A" />
        {[["most nincs időm", 80, 56, 0], ["majd holnap", 176, 42, 0.7], ["fáj a térdem", 250, 66, 1.4]].map(([t, x, y, d], k) => (
          <g key={k} style={{ transformOrigin: `${x}px ${y}px`, animation: `m11rise 2.8s ease-in-out ${d}s infinite` }}>
            <ellipse cx={x} cy={y} rx="50" ry="24" fill="#fff" stroke="#CBD5E1" strokeWidth="2" filter="url(#soft)" />
            <ellipse cx={x - 28} cy={y + 20} rx="8" ry="6" fill="#fff" stroke="#CBD5E1" strokeWidth="2" />
            <ellipse cx={x - 38} cy={y + 30} rx="4" ry="3" fill="#fff" stroke="#CBD5E1" strokeWidth="1.5" />
            <text x={x} y={y + 4} textAnchor="middle" fontSize="12" fontWeight="700" fill="#64748B">{t}</text>
          </g>
        ))}
        <style>{`@keyframes m11rise{0%{opacity:0;transform:translateY(16px) scale(.9)}22%{opacity:1;transform:translateY(0) scale(1)}68%{opacity:1}84%{opacity:0;transform:scale(1.25)}100%{opacity:0}}`}</style>
      </>),
    ...
    /* 14 — kanapé pulzáló gravitációs mezővel, beleszippantott figura, nyilak */
    14: wrap(
      <>
      ...
      <>
        ...
        {[120, 160, 200].map((x, k) => (
          <g key={x} style={{ animation: `m14pull 1.5s ease-in ${k * 0.2}s infinite` }}>
            <line x1={x} y1="40" x2={x} y2="70" stroke="#7E57C2" strokeWidth="3.5" strokeLinecap="round" />
            <polygon points={`${x - 6},66 ${x + 6},66 ${x},78`} fill="#7E57C2" />
          </g>
        ))}
        <rect x="74" y="120" width="172" height="50" rx="15" fill="url(#couch2)" filter="url(#soft)" />
        <rect x="66" y="98" width="28" height="72" rx="12" fill="#8877C2" />
        <rect x="226" y="98" width="28" height="72" rx="12" fill="#8877C2" />
        <rect x="90" y="110" width="140" height="18" rx="8" fill="#A99AD8" />
        <ellipse cx="160" cy="130" rx="58" ry="18" fill="url(#skin)" />
        <ellipse cx="178" cy="126" rx="24" ry="13" fill="#F0B98A" />
        <circle cx="112" cy="116" r="16" fill="url(#skin)" />
        <circle cx="112" cy="116" r="16" fill="url(#skinHi)" />
        <path d="M99 110 a16 11 0 0 1 30 0z" fill="#4E342E" />
        <line x1="105" y1="118" x2="117" y2="118" stroke="#2B2118" strokeWidth="2" strokeLinecap="round" />
        <path d="M105 126 q6 3 12 0" stroke="#8A3B3B" strokeWidth="1.8" fill="none" strokeLinecap="round" />
