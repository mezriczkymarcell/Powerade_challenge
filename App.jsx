...
/* Minden motivációhoz egy kis animált SVG jelenet. index 0..14 */
function MotivArt({ i }) {
  const box = "0 0 200 120";
  const S = ({ children }) => (
    <svg viewBox={box} width="100%" height="120" style={{ display: "block" }}>{children}</svg>
  );
  const sky = <rect x="0" y="0" width="200" height="120" fill="#EEF4FA" />;

  const arts = {
    // 0 – kerülés vs átugrás: egy pálcikaember körbejár egy hatalmas hasat
    0: (<S>{sky}
      <ellipse cx="100" cy="80" rx="42" ry="34" fill="#F4C89A" />
      <circle cx="100" cy="46" r="12" fill="#F4C89A" />
      <g style={{ transformOrigin: "100px 80px", animation: "spin 3s linear infinite" }}>
        <circle cx="150" cy="80" r="6" fill={C.blue} />
        <line x1="150" y1="86" x2="150" y2="100" stroke={C.blue} strokeWidth="3" />
      </g>
      <style>{`@keyframes spin{to{transform:rotate(360deg)}}`}</style>
    </S>),
    // 1 – daruskocsi emel egy koporsót
    1: (<S>{sky}
      <line x1="30" y1="20" x2="30" y2="95" stroke="#555" strokeWidth="5" />
      <line x1="30" y1="22" x2="140" y2="22" stroke="#555" strokeWidth="5" />
      <line x1="140" y1="22" x2="140" y2="45" stroke="#999" strokeWidth="2" />
      <g style={{ animation: "bob2 2s ease-in-out infinite" }}>
        <rect x="120" y="45" width="42" height="20" rx="2" fill="#6B4423" />
        <rect x="136" y="48" width="10" height="6" fill="#B8860B" />
      </g>
      <rect x="14" y="95" width="60" height="18" rx="3" fill="#F5A623" />
      <circle cx="28" cy="115" r="7" fill="#333" /><circle cx="60" cy="115" r="7" fill="#333" />
      <style>{`@keyframes bob2{0%,100%{transform:translateY(0)}50%{transform:translateY(-6px)}}`}</style>
    </S>),
    // 2 – sárgadinnye a póló alatt
    2: (<S>{sky}
      <circle cx="100" cy="40" r="13" fill="#F4C89A" />
      <path d="M78 60 q22 -8 44 0 l6 44 q-28 10 -56 0 z" fill={C.blue} />
      <g style={{ transformOrigin: "100px 82px", animation: "pulse 1.4s ease-in-out infinite" }}>
        <ellipse cx="100" cy="82" rx="26" ry="22" fill="#7CB342" />
        <path d="M80 82 q20 -14 40 0" stroke="#558B2F" strokeWidth="2" fill="none" />
        <path d="M80 82 q20 14 40 0" stroke="#558B2F" strokeWidth="2" fill="none" />
      </g>
      <style>{`@keyframes pulse{0%,100%{transform:scale(1)}50%{transform:scale(1.08)}}`}</style>
    </S>),
    // 3 – szomszéd integet a kerítésnél
    3: (<S>{sky}
      {[40,60,80,100,120,140,160].map(x=><rect key={x} x={x} y="70" width="8" height="40" fill="#C89B6B"/>)}
      <rect x="34" y="78" width="140" height="6" fill="#C89B6B" />
      <circle cx="150" cy="58" r="12" fill="#F4C89A" />
      <g style={{ transformOrigin: "165px 60px", animation: "wave 0.8s ease-in-out infinite" }}>
        <line x1="160" y1="66" x2="176" y2="50" stroke="#F4C89A" strokeWidth="5" strokeLinecap="round" />
      </g>
      <circle cx="55" cy="52" r="12" fill="#F4C89A" />
      <path d="M48 52 q7 -6 14 0" stroke="#B5715A" strokeWidth="2" fill="none" />
      <style>{`@keyframes wave{0%,100%{transform:rotate(-18deg)}50%{transform:rotate(18deg)}}`}</style>
    </S>),
    // 4 – ugyanaz mint 3 (a párod motívum) – kanapé + tv
    4: (<S>{sky}
      <rect x="30" y="70" width="90" height="34" rx="8" fill="#8E7CC3" />
      <rect x="26" y="60" width="18" height="44" rx="6" fill="#7E6BB0" />
      <circle cx="80" cy="60" r="12" fill="#F4C89A" />
      <ellipse cx="80" cy="90" rx="26" ry="18" fill="#F4C89A" opacity="0.85" />
      <rect x="140" y="50" width="44" height="30" rx="3" fill="#222" />
      <rect x="144" y="54" width="36" height="22" fill={C.cyan} style={{ animation: "flick 0.4s steps(2) infinite" }} />
      <style>{`@keyframes flick{0%{opacity:1}50%{opacity:.6}}`}</style>
    </S>),
    // 5 – tükör: kis izom vs valóság
    5: (<S>{sky}
      <circle cx="100" cy="40" r="14" fill="#F4C89A" />
      <ellipse cx="100" cy="82" rx="34" ry="26" fill="#F0B27A" />
      <g style={{ animation: "flex 1.2s ease-in-out infinite" }}>
        <circle cx="66" cy="70" r="7" fill="#F4C89A" />
        <circle cx="134" cy="70" r="7" fill="#F4C89A" />
      </g>
      <style>{`@keyframes flex{0%,100%{transform:translateY(0)}50%{transform:translateY(-3px)}}`}</style>
    </S>),
    // 6 – szív EKG vonallal
    6: (<S>{sky}
      <path d="M60 100 q-24 -30 0 -48 q22 -18 40 8 q18 -26 40 -8 q24 18 0 48 q-20 18 -40 34 q-20 -16 -40 -34 z" fill="#E53935"
        style={{ transformOrigin:"100px 80px", animation:"beat 0.7s ease-in-out infinite" }} />
      <polyline points="10,60 40,60 50,40 60,80 70,60 190,60" fill="none" stroke="#fff" strokeWidth="3" />
      <style>{`@keyframes beat{0%,100%{transform:scale(1)}30%{transform:scale(1.12)}}`}</style>
    </S>),
    // 7 – teherautó "DEATH" felirattal
    7: (<S>{sky}
      <g style={{ animation: "drive 3s linear infinite" }}>
        <rect x="20" y="60" width="90" height="34" rx="3" fill="#37474F" />
        <rect x="110" y="70" width="34" height="24" rx="3" fill="#546E7A" />
        <text x="65" y="82" textAnchor="middle" fontSize="14" fontWeight="900" fill="#fff">HALÁL</text>
        <circle cx="45" cy="98" r="9" fill="#111" /><circle cx="120" cy="98" r="9" fill="#111" />
      </g>
      <style>{`@keyframes drive{0%{transform:translateX(-30px)}50%{transform:translateX(30px)}100%{transform:translateX(-30px)}}`}</style>
    </S>),
    // 8 – síró futó
    8: (<S>{sky}
      <circle cx="100" cy="44" r="14" fill="#F4C89A" />
      <path d="M92 48 q-2 20 -2 26" stroke={C.cyan} strokeWidth="3" fill="none" style={{ animation:"tear 1s linear infinite" }} />
      <path d="M108 48 q2 20 2 26" stroke={C.cyan} strokeWidth="3" fill="none" style={{ animation:"tear 1s linear .3s infinite" }} />
      <g style={{ transformOrigin:"100px 80px", animation:"run 0.5s ease-in-out infinite" }}>
        <line x1="100" y1="58" x2="100" y2="86" stroke="#333" strokeWidth="4" />
        <line x1="100" y1="86" x2="86" y2="104" stroke="#333" strokeWidth="4" />
        <line x1="100" y1="86" x2="114" y2="104" stroke="#333" strokeWidth="4" />
      </g>
      <style>{`@keyframes tear{0%{opacity:1}100%{opacity:0;transform:translateY(10px)}}@keyframes run{0%,100%{transform:rotate(-4deg)}50%{transform:rotate(4deg)}}`}</style>
    </S>),
    // 9 – omladozó épület
    9: (<S>{sky}
      <rect x="55" y="45" width="90" height="65" fill="#9E9E9E" />
      {[0,1,2].map(r=>[0,1,2,3].map(c=>(
        <rect key={`${r}-${c}`} x={62+c*20} y={52+r*18} width="12" height="12" fill="#607D8B"
          style={{ animation:`crumble 2s ease-in-out ${(r+c)*0.2}s infinite` }} />
      )))}
      <polygon points="55,45 100,28 145,45" fill="#795548" />
      <style>{`@keyframes crumble{0%,100%{opacity:1;transform:translateY(0)}50%{opacity:.5;transform:translateY(3px)}}`}</style>
    </S>),
    // 10 – felfelé kúszó koleszterin-görbe
    10: (<S>{sky}
      <line x1="20" y1="105" x2="185" y2="105" stroke="#999" strokeWidth="2" />
      <line x1="20" y1="105" x2="20" y2="20" stroke="#999" strokeWidth="2" />
      <polyline points="20,100 60,90 100,70 140,40 180,20" fill="none" stroke="#E53935" strokeWidth="4"
        strokeDasharray="240" style={{ animation:"draw 2s ease-out infinite" }} />
      <text x="150" y="18" fontSize="11" fontWeight="900" fill="#E53935">LDL</text>
      <style>{`@keyframes draw{from{stroke-dashoffset:240}to{stroke-dashoffset:0}}`}</style>
    </S>),
    // 11 – kifogás-buborékok elpukkannak
    11: (<S>{sky}
      {[[50,70],[100,55],[150,75]].map(([x,y],i)=>(
        <g key={i} style={{ animation:`pop 1.6s ease-in-out ${i*0.4}s infinite` }}>
          <ellipse cx={x} cy={y} rx="26" ry="18" fill="#fff" stroke="#CBD5E1" strokeWidth="2" />
          <text x={x} y={y+4} textAnchor="middle" fontSize="9" fill="#94A3B8">nem érek rá</text>
        </g>
      ))}
      <style>{`@keyframes pop{0%,60%{opacity:1;transform:scale(1)}80%{opacity:0;transform:scale(1.3)}100%{opacity:0}}`}</style>
    </S>),
    // 12 – GPS "újratervezés"
    12: (<S>{sky}
      <circle cx="100" cy="65" r="34" fill="#fff" stroke={C.blue} strokeWidth="3" />
      <g style={{ transformOrigin:"100px 65px", animation:"spin 2s linear infinite" }}>
        <polygon points="100,45 108,72 100,66 92,72" fill={C.blue} />
      </g>
      <text x="100" y="112" textAnchor="middle" fontSize="12" fontWeight="900" fill={C.danger}>Újratervezés…</text>
      <style>{`@keyframes spin{to{transform:rotate(360deg)}}`}</style>
    </S>),
    // 13 – önbecsülés-mérce lefelé
    13: (<S>{sky}
      <rect x="90" y="20" width="20" height="86" rx="10" fill="#ddd" />
      <rect x="90" y="70" width="20" height="36" rx="10" fill={C.blue}
        style={{ animation:"drop 2s ease-in-out infinite" }} />
      <text x="120" y="40" fontSize="10" fill="#94A3B8">önbecsülés</text>
      <style>{`@keyframes drop{0%,100%{height:36px;y:70px}50%{height:16px;y:90px}}`}</style>
    </S>),
    // 14 – kanapé gravitációs mezővel
    14: (<S>{sky}
      <ellipse cx="100" cy="95" rx="60" ry="14" fill="#B39DDB" opacity="0.4"
        style={{ animation:"pulse2 1.6s ease-in-out infinite" }} />
      <rect x="55" y="72" width="90" height="30" rx="8" fill="#8E7CC3" />
      <circle cx="100" cy="64" r="12" fill="#F4C89A" />
      <ellipse cx="100" cy="88" rx="24" ry="14" fill="#F4C89A" opacity="0.8" />
      <style>{`@keyframes pulse2{0%,100%{transform:scale(1);opacity:.4}50%{transform:scale(1.15);opacity:.6}}`}</style>
    </S>),
  };
  return arts[i] || arts[0];
}
      ...
      {open && (
        <div className="fixed inset-0 z-[78] flex items-center justify-center px-5"
          ...
          <Card className="w-full max-w-sm overflow-hidden p-0" onClick={(e) => e.stopPropagation()}
            ...
            <div style={{ background: queen
              ? "linear-gradient(135deg,#4A0D3F,#C42A6B)"
              : "linear-gradient(135deg,#1B1246,#6D28D9)" }}>
              <MotivArt i={i} />
            </div>
