...
/* ==================== A KIRÁLY MOTIVÁCIÓI (easter egg) =================== */
// Minden elem: a szöveg és a hozzá tartozó illusztráció INDEXE egy helyen,
// hogy fizikailag ne csúszhasson el. (art = a MotivArt jelenet indexe)
const MOTIVATIONS = [
  { art: 0,  text: "Könnyebb téged megkerülni, mint átugrani. Mozogj már!" },
  { art: 1,  text: "Ha így folytatod, a koporsódat nem hordárok, hanem egy darus kocsi fogja vinni." },
  { art: 2,  text: "A sörhas nem tekintély, hanem egy lassan ölő sárgadinnye a pólód alatt. Emeld fel a segged!" },
  { art: 3,  text: "Fuss egy kört! A párodnak is kell egy kis egyedüllét a szomszéddal." },
  { art: 5,  text: "A zsírod nem melegít, csak eltakarja azt a minimális férfiasságodat, ami még megmaradt." },
  { art: 6,  text: "Ha ma sem edzel, holnapra megint egy lépéssel közelebb kerülsz az infarktushoz. Egészségedre!" },
  { art: 7,  text: "A halál mindannyiunkért eljön, de érted valószínűleg teherszállítóval kell jönnie." },
  { art: 8,  text: "Sírva is lehet futni, ráadásul a könnyekkel is vizet veszítesz. Nyomás!" },
  { art: 9,  text: "A tested egy templom. De jelenleg inkább egy omladozó, elhagyatott szovjet laktanyára hasonlít." },
  { art: 10, text: "Az egyetlen dolog, ami gyorsabban nő benned a motivációnál, az a koleszterinszinted." },
  { art: 11, text: "A kifogások nem égetnek kalóriát. Indulás!" },
  { art: 12, text: "Még a GPS is azt mondja rád, hogy „újratervezés”, amikor meglátja a sziluettedet." },
  { art: 13, text: "Ha most elindulsz futni, talán még azelőtt hazaérsz, hogy teljesen elhagynád az önbecsülésed." },
  { art: 14, text: "Nem vagy te lusta, csak a saját gravitációs meződ tart fogva a kanapén. Törj ki belőle!" },
  { art: 15, text: "Fuss el a problémáid elől. Vagy legalább a korai szívhalál elől." },
];

/* Minden motivációhoz egy gazdagon kidolgozott, animált SVG jelenet (0..14). */
function MotivArt({ i }) {
  const wrap = (extraDefs, children) => (
    <svg viewBox="0 0 320 200" width="100%" height="196" style={{ display: "block" }}>
      <defs>
        {/* --- közös festékkészlet --- */}
        <linearGradient id="skin" x1="0" y1="0" x2="0.9" y2="1">
          <stop offset="0%" stopColor="#FCE0BE" /><stop offset="55%" stopColor="#F3C393" /><stop offset="100%" stopColor="#DCA36C" />
        </linearGradient>
        <radialGradient id="skinHi" cx="0.38" cy="0.32" r="0.7">
          <stop offset="0%" stopColor="#fff" stopOpacity="0.55" /><stop offset="60%" stopColor="#fff" stopOpacity="0" />
        </radialGradient>
        <linearGradient id="flame" x1="0" y1="1" x2="0" y2="0">
          <stop offset="0%" stopColor="#FF2D00" /><stop offset="45%" stopColor="#FF7A00" /><stop offset="80%" stopColor="#FFC400" /><stop offset="100%" stopColor="#FFF59D" />
        </linearGradient>
  ...
  const shadow = (cx, cy, rx) => <ellipse cx={cx} cy={cy} rx={rx} ry={rx * 0.14} fill="#0F172A" opacity="0.10" />;

  const A = {
    /* 0 — pocakos óriás, körülötte lihegve futó pálcikaember (nem tudja átugrani) */
    0: wrap(
      <radialGradient id="bellyG" cx="0.4" cy="0.35" r="0.8">
        <stop offset="0%" stopColor="#FBD8B2" /><stop offset="100%" stopColor="#D89E63" />
      </radialGradient>,
      <>
        {shadow(160, 186, 120)}
        {/* lábak */}
        <path d="M120 150 q-6 24 -3 40 l16 0 q2 -18 6 -34z" fill="url(#skin)" />
        <path d="M200 150 q6 24 3 40 l-16 0 q-2 -18 -6 -34z" fill="url(#skin)" />
        <ellipse cx="126" cy="192" rx="16" ry="5" fill="#3A3A3A" /><ellipse cx="194" cy="192" rx="16" ry="5" fill="#3A3A3A" />
        {/* has */}
        <ellipse cx="160" cy="120" rx="74" ry="60" fill="url(#bellyG)" filter="url(#soft)" />
        <ellipse cx="160" cy="120" rx="74" ry="60" fill="url(#skinHi)" />
        <path d="M120 120 q40 26 80 0" stroke="#C88A54" strokeWidth="2" fill="none" opacity="0.5" />
        <ellipse cx="160" cy="132" rx="4" ry="8" fill="#B87840" opacity="0.6" />
        {/* mellkas + fej */}
        <ellipse cx="160" cy="70" rx="34" ry="24" fill="url(#skin)" />
        <circle cx="160" cy="46" r="25" fill="url(#skin)" />
        <circle cx="160" cy="46" r="25" fill="url(#skinHi)" />
        <path d="M137 40 a24 18 0 0 1 46 0 q-23 -12 -46 0z" fill="#4A3222" />
        <ellipse cx="150" cy="47" rx="2.8" ry="3.2" fill="#2B2118" /><ellipse cx="170" cy="47" rx="2.8" ry="3.2" fill="#2B2118" />
        <circle cx="151" cy="46" r="0.9" fill="#fff" /><circle cx="171" cy="46" r="0.9" fill="#fff" />
        <path d="M150 57 q10 6 20 0" stroke="#B5715A" strokeWidth="2.4" fill="none" strokeLinecap="round" />
        <circle cx="141" cy="53" r="4" fill="#F19B9B" opacity="0.45" /><circle cx="179" cy="53" r="4" fill="#F19B9B" opacity="0.45" />
        {/* orbitáló futó */}
        <g style={{ transformOrigin: "160px 120px", animation: "m0orbit 3.6s linear infinite" }}>
          <g transform="translate(250 120)">
            <ellipse cx="0" cy="30" rx="12" ry="3" fill="#0F172A" opacity="0.15" />
            <circle cx="0" cy="-22" r="9" fill="#0F172A" />
            <circle cx="-2.5" cy="-23" r="2.2" fill="#fff" opacity="0.5" />
            <line x1="0" y1="-13" x2="0" y2="10" stroke="#0F172A" strokeWidth="5.5" strokeLinecap="round" />
            <g style={{ transformOrigin: "0px 10px", animation: "m0legs 0.42s ease-in-out infinite" }}>
              <line x1="0" y1="10" x2="-10" y2="27" stroke="#0F172A" strokeWidth="5.5" strokeLinecap="round" />
            </g>
            <line x1="0" y1="10" x2="10" y2="25" stroke="#0F172A" strokeWidth="5.5" strokeLinecap="round" />
            <line x1="0" y1="-6" x2="-13" y2="3" stroke="#0F172A" strokeWidth="4.5" strokeLinecap="round" />
            <line x1="0" y1="-6" x2="13" y2="0" stroke="#0F172A" strokeWidth="4.5" strokeLinecap="round" />
            {/* izzadság */}
            <circle cx="8" cy="-30" r="2.4" fill="#4FC3F7" style={{ animation: "m0sweat 0.9s ease-out infinite" }} />
          </g>
        </g>
        <style>{`@keyframes m0orbit{to{transform:rotate(360deg)}}@keyframes m0legs{0%,100%{transform:rotate(-18deg)}50%{transform:rotate(18deg)}}@keyframes m0sweat{0%{opacity:0;transform:translate(0,0)}30%{opacity:1}100%{opacity:0;transform:translate(6px,10px)}}`}</style>
      </>),

    /* 1 — sárga mobildaru láncon lengő koporsóval */
    1: wrap(
      <>
        <linearGradient id="craneY" x1="0" y1="0" x2="0" y2="1">
          <stop offset="0%" stopColor="#FFD54F" /><stop offset="100%" stopColor="#F5A623" />
        </linearGradient>
        <linearGradient id="woodC" x1="0" y1="0" x2="1" y2="1">
          <stop offset="0%" stopColor="#8D5A2B" /><stop offset="100%" stopColor="#5D3A18" />
        </linearGradient>
      </>,
      <>
        {shadow(180, 190, 140)}
        {/* torony rácsszerkezet */}
        <rect x="40" y="34" width="14" height="118" rx="2" fill="url(#craneY)" />
        {Array.from({ length: 9 }).map((_, k) => (
          <line key={k} x1="40" y1={40 + k * 13} x2="54" y2={53 + k * 13} stroke="#B8860B" strokeWidth="1.6" opacity="0.7" />
        ))}
        {/* gém */}
        <polygon points="40,34 40,20 66,34" fill="url(#craneY)" />
        <rect x="46" y="30" width="210" height="10" rx="3" fill="url(#craneY)" transform="rotate(-1 46 34)" />
        {Array.from({ length: 8 }).map((_, k) => (
          <line key={k} x1={60 + k * 24} y1="31" x2={72 + k * 24} y2="41" stroke="#B8860B" strokeWidth="1.4" opacity="0.6" />
        ))}
        <line x1="244" y1="34" x2="244" y2="66" stroke="#7B7B7B" strokeWidth="2.5" />
        {/* lengő koporsó */}
        <g style={{ transformOrigin: "244px 34px", animation: "m1swing 2.8s ease-in-out infinite" }}>
          <line x1="244" y1="34" x2="244" y2="70" stroke="#8A8A8A" strokeWidth="2.5" />
          <g filter="url(#soft)">
            <polygon points="214,70 274,70 266,132 222,132" fill="url(#woodC)" />
            <polygon points="214,70 274,70 269,92 219,92" fill="#A56A34" opacity="0.5" />
            <rect x="236" y="86" width="16" height="7" rx="1.5" fill="#E7C64B" />
            <rect x="240" y="80" width="8" height="24" rx="2" fill="#E7C64B" />
            <ellipse cx="244" cy="83" rx="3" ry="3" fill="#F5E08A" />
          </g>
        </g>
        {/* alváz */}
        <rect x="150" y="142" width="132" height="30" rx="6" fill="url(#craneY)" filter="url(#soft)" />
        <rect x="150" y="142" width="132" height="9" rx="6" fill="#fff" opacity="0.3" />
        <path d="M258 118 h30 a6 6 0 0 1 6 6 v18 h-36 z" fill="url(#craneY)" />
        <rect x="264" y="123" width="24" height="15" rx="2" fill="#B3E5FC" />
        <rect x="264" y="123" width="24" height="15" rx="2" fill="none" stroke="#0288D1" strokeWidth="1" />
        {/* stabilizátor */}
        <path d="M150 168 l-16 14 6 4 20 -12z" fill="#455A64" />
        {[178, 246].map((cx) => (
          <g key={cx}>
            <circle cx={cx} cy="174" r="15" fill="#1A1A1A" />
            <circle cx={cx} cy="174" r="14" fill="none" stroke="#333" strokeWidth="2" />
            <g style={{ transformOrigin: `${cx}px 174px`, animation: "m1wheel 1.4s linear infinite" }}>
              <circle cx={cx} cy="174" r="6" fill="#666" />
              {[0, 60, 120].map((a) => (
                <line key={a} x1={cx} y1="174" x2={cx + Math.cos(a * Math.PI / 180) * 13} y2={174 + Math.sin(a * Math.PI / 180) * 13} stroke="#444" strokeWidth="2" />
              ))}
            </g>
          </g>
        ))}
        <style>{`@keyframes m1swing{0%,100%{transform:rotate(-7deg)}50%{transform:rotate(7deg)}}@keyframes m1wheel{to{transform:rotate(360deg)}}`}</style>
      </>),

    /* 2 — feszülő póló alatt pulzáló, texturált sárgadinnye */
    2: wrap(
      <radialGradient id="melonG" cx="0.4" cy="0.35" r="0.75">
        <stop offset="0%" stopColor="#AED581" /><stop offset="70%" stopColor="#7CB342" /><stop offset="100%" stopColor="#558B2F" />
      </radialGradient>,
      <>
        {shadow(160, 188, 74)}
        <circle cx="160" cy="46" r="25" fill="url(#skin)" />
        <circle cx="160" cy="46" r="25" fill="url(#skinHi)" />
        <path d="M136 40 a24 17 0 0 1 48 0 q-24 -10 -48 0z" fill="#3E2723" />
        <ellipse cx="151" cy="47" rx="2.6" ry="3" fill="#2B2118" /><ellipse cx="169" cy="47" rx="2.6" ry="3" fill="#2B2118" />
        <path d="M151 57 q9 6 18 0" stroke="#B5715A" strokeWidth="2.4" fill="none" strokeLinecap="round" />
        {/* póló */}
        <path d="M116 86 q44 -16 88 0 l14 96 q-56 18 -116 0z" fill="#2F6FE0" filter="url(#soft)" />
        <path d="M116 86 q44 -16 88 0" fill="none" stroke="#1E4FA0" strokeWidth="3" />
        <path d="M138 84 l22 20 22 -20" fill="none" stroke="#1E4FA0" strokeWidth="2.5" opacity="0.6" />
        {/* ránc-vonalak a feszüléstől */}
        <path d="M118 140 q42 -8 84 0" stroke="#1E4FA0" strokeWidth="1.6" fill="none" opacity="0.4" />
        <path d="M120 156 q40 -6 80 0" stroke="#1E4FA0" strokeWidth="1.6" fill="none" opacity="0.3" />
        {/* dinnye */}
        <g style={{ transformOrigin: "160px 142px", animation: "m2pulse 1.5s ease-in-out infinite" }}>
          <ellipse cx="160" cy="142" rx="48" ry="42" fill="url(#melonG)" />
          {[-30, -15, 0, 15, 30].map((dx) => (
            <path key={dx} d={`M${160 + dx} 102 q${-dx * 0.5} 40 0 80`} stroke="#4E7A2A" strokeWidth="2.2" fill="none" opacity="0.55" />
          ))}
          <ellipse cx="144" cy="128" rx="12" ry="7" fill="#fff" opacity="0.3" />
        </g>
        <style>{`@keyframes m2pulse{0%,100%{transform:scale(1)}50%{transform:scale(1.07)}}`}</style>
      </>),

    /* 3 — kerítés fölött vigyorgó, integető szomszéd; a "te" fejed leselkedik */
    3: wrap(
      <linearGradient id="fenceG" x1="0" y1="0" x2="0" y2="1">
        <stop offset="0%" stopColor="#D8A76A" /><stop offset="100%" stopColor="#B07C42" />
      </linearGradient>,
      <>
        <rect x="0" y="150" width="320" height="50" fill="#8BC34A" opacity="0.22" />
        {Array.from({ length: 11 }).map((_, k) => (
          <g key={k}>
            <rect x={10 + k * 28} y="92" width="18" height="88" rx="2" fill="url(#fenceG)" />
            <polygon points={`${10 + k * 28},92 ${19 + k * 28},82 ${28 + k * 28},92`} fill="url(#fenceG)" />
            <line x1={14 + k * 28} y1="98" x2={14 + k * 28} y2="176" stroke="#8A5E2E" strokeWidth="1" opacity="0.5" />
          </g>
        ))}
        <rect x="4" y="106" width="312" height="9" fill="#A9754A" /><rect x="4" y="148" width="312" height="9" fill="#A9754A" />
        {/* szomszéd */}
        <g filter="url(#soft)">
          <circle cx="238" cy="70" r="27" fill="url(#skin)" />
          <circle cx="238" cy="70" r="27" fill="url(#skinHi)" />
          <path d="M214 62 a26 17 0 0 1 50 0 q-25 -8 -50 0z" fill="#6D4C41" />
          <path d="M214 62 q4 -14 24 -15" stroke="#6D4C41" strokeWidth="3" fill="none" opacity="0.6" />
          <ellipse cx="229" cy="70" rx="2.8" ry="3.2" fill="#2B2118" /><ellipse cx="247" cy="70" rx="2.8" ry="3.2" fill="#2B2118" />
          <path d="M226 82 q12 9 24 0" stroke="#8A3B3B" strokeWidth="3" fill="none" strokeLinecap="round" />
          <path d="M226 82 q12 9 24 0" stroke="#fff" strokeWidth="1.4" fill="none" strokeLinecap="round" opacity="0.5" transform="translate(0 -1)" />
          <circle cx="222" cy="78" r="4.5" fill="#F19B9B" opacity="0.5" /><circle cx="254" cy="78" r="4.5" fill="#F19B9B" opacity="0.5" />
        </g>
        {/* integető kar */}
        <g style={{ transformOrigin: "266px 92px", animation: "m3wave 0.7s ease-in-out infinite" }}>
          <line x1="266" y1="92" x2="292" y2="60" stroke="url(#skin)" strokeWidth="10" strokeLinecap="round" />
          <circle cx="294" cy="56" r="8" fill="url(#skin)" />
        </g>
        {/* leselkedő fejtető */}
        <path d="M56 108 a22 20 0 0 1 44 0z" fill="url(#skin)" />
        <path d="M56 106 a22 16 0 0 1 44 0 q-22 -8 -44 0z" fill="#3E2723" />
        <ellipse cx="70" cy="105" rx="2.4" ry="2.8" fill="#2B2118" /><ellipse cx="86" cy="105" rx="2.4" ry="2.8" fill="#2B2118" />
        <style>{`@keyframes m3wave{0%,100%{transform:rotate(-22deg)}50%{transform:rotate(24deg)}}`}</style>
      </>),

    /* 4 — kanapén elomló figura, villódzó tévé, sötét szoba */
    4: wrap(
      <>
        <linearGradient id="couchG" x1="0" y1="0" x2="0" y2="1">
          <stop offset="0%" stopColor="#9D8DD1" /><stop offset="100%" stopColor="#7E6BB0" />
        </linearGradient>
        <radialGradient id="tvGlow" cx="0.5" cy="0.5" r="0.7">
          <stop offset="0%" stopColor="#4FC3F7" /><stop offset="100%" stopColor="#0277BD" />
        </radialGradient>
      </>,
      <>
        {shadow(140, 188, 130)}
        {/* fénykúp a tv-ből */}
        <polygon points="270,96 300,80 300,140 270,124" fill="#4FC3F7" opacity="0.12" />
        {/* kanapé */}
        <rect x="24" y="108" width="188" height="58" rx="16" fill="url(#couchG)" filter="url(#soft)" />
        <rect x="16" y="82" width="32" height="84" rx="14" fill="#8877C2" />
        <rect x="188" y="82" width="32" height="84" rx="14" fill="#8877C2" />
        <rect x="40" y="98" width="156" height="20" rx="9" fill="#A99AD8" />
        <path d="M60 118 h120" stroke="#6E5CA6" strokeWidth="2" opacity="0.5" />
        {/* elomló figura */}
        <ellipse cx="122" cy="122" rx="64" ry="20" fill="url(#skin)" />
        <ellipse cx="135" cy="112" rx="30" ry="17" fill="#F0B98A" />
        <circle cx="70" cy="106" r="19" fill="url(#skin)" />
        <circle cx="70" cy="106" r="19" fill="url(#skinHi)" />
        <path d="M53 100 a19 13 0 0 1 36 0z" fill="#4E342E" />
        <line x1="60" y1="110" x2="72" y2="110" stroke="#2B2118" strokeWidth="2" strokeLinecap="round" />
        <path d="M60 118 q6 3 12 0" stroke="#8A3B3B" strokeWidth="2" fill="none" strokeLinecap="round" />
        {/* lelógó kar + kézben távirányító */}
        <line x1="150" y1="128" x2="168" y2="150" stroke="url(#skin)" strokeWidth="9" strokeLinecap="round" />
        <rect x="164" y="150" width="9" height="18" rx="2" fill="#333" transform="rotate(12 168 158)" />
        {/* tv */}
        <rect x="236" y="70" width="68" height="48" rx="5" fill="#1A1A1A" filter="url(#soft)" />
        <rect x="242" y="76" width="56" height="36" rx="2" fill="url(#tvGlow)" style={{ animation: "m4flick 0.5s steps(3) infinite" }} />
        <rect x="248" y="82" width="20" height="4" rx="2" fill="#fff" opacity="0.4" />
        <rect x="248" y="92" width="34" height="4" rx="2" fill="#fff" opacity="0.25" />
        <rect x="262" y="118" width="16" height="12" fill="#1A1A1A" />
        <rect x="248" y="130" width="44" height="5" rx="2" fill="#1A1A1A" />
        <style>{`@keyframes m4flick{0%{opacity:1}33%{opacity:.5}66%{opacity:.85}}`}</style>
      </>),

    /* 5 — tükör előtt pici bicepszet mutató, izzadó alak */
    5: wrap(
      <linearGradient id="mirrorG" x1="0" y1="0" x2="1" y2="1">
        <stop offset="0%" stopColor="#E3F2FD" /><stop offset="100%" stopColor="#BBDEFB" />
      </linearGradient>,
      <>
        {shadow(120, 186, 80)}
        <ellipse cx="120" cy="130" rx="48" ry="44" fill="url(#skin)" filter="url(#soft)" />
        <ellipse cx="120" cy="130" rx="48" ry="44" fill="url(#skinHi)" />
        <ellipse cx="120" cy="138" rx="4" ry="7" fill="#C88A54" opacity="0.5" />
        <circle cx="120" cy="58" r="25" fill="url(#skin)" />
        <circle cx="120" cy="58" r="25" fill="url(#skinHi)" />
        <path d="M97 52 a24 17 0 0 1 46 0 q-23 -9 -46 0z" fill="#4E342E" />
        <ellipse cx="111" cy="58" rx="2.6" ry="3" fill="#2B2118" /><ellipse cx="129" cy="58" rx="2.6" ry="3" fill="#2B2118" />
        <path d="M111 68 q9 5 18 0" stroke="#8A3B3B" strokeWidth="2.4" fill="none" strokeLinecap="round" />
        {/* felkar + pici bicepsz */}
        <g style={{ transformOrigin: "148px 118px", animation: "m5flex 1.3s ease-in-out infinite" }}>
          <path d="M148 120 q30 -6 34 -34 q-12 -4 -22 10 q-6 10 -16 16z" fill="url(#skin)" />
          <ellipse cx="166" cy="90" rx="8" ry="7" fill="#F0B98A" />
          <ellipse cx="164" cy="88" rx="3" ry="2" fill="#fff" opacity="0.4" />
        </g>
        {/* izzadságcseppek */}
        {[[98, 44], [142, 46], [120, 40]].map(([cx, cy], k) => (
          <path key={k} d={`M${cx} ${cy} q-3 5 0 8 q3 -3 0 -8z`} fill="#4FC3F7"
            style={{ animation: `m5sweat 1.4s ease-out ${k * 0.4}s infinite` }} />
        ))}
        {/* tükör */}
        <rect x="212" y="36" width="92" height="128" rx="10" fill="url(#mirrorG)" />
        <rect x="212" y="36" width="92" height="128" rx="10" fill="none" stroke="#90A4AE" strokeWidth="6" />
        <polygon points="224,50 248,50 216,150 200,150" fill="#fff" opacity="0.35" />
        <style>{`@keyframes m5flex{0%,100%{transform:rotate(0)}50%{transform:rotate(-9deg)}}@keyframes m5sweat{0%{opacity:0;transform:translateY(0)}30%{opacity:1}100%{opacity:0;transform:translateY(22px)}}`}</style>
      </>),

    /* 6 — nagy, fényes dobogó szív + kirajzolódó EKG */
    6: wrap(
      <radialGradient id="heartG" cx="0.38" cy="0.3" r="0.8">
        <stop offset="0%" stopColor="#FF6B6B" /><stop offset="60%" stopColor="#E53935" /><stop offset="100%" stopColor="#B71C1C" />
      </radialGradient>,
      <>
        <rect x="0" y="98" width="320" height="3" fill="#CFD8DC" opacity="0.6" />
        <polyline points="0,99 84,99 104,58 122,138 142,30 162,128 182,99 320,99"
          fill="none" stroke="#EF5350" strokeWidth="4.5" strokeLinecap="round" strokeLinejoin="round"
          style={{ strokeDasharray: 760, animation: "m6ekg 2.2s linear infinite" }} />
        <g style={{ transformOrigin: "232px 100px", animation: "m6beat 0.78s ease-in-out infinite" }}>
          <path d="M232 158 C186 122 188 74 214 66 C232 60 246 74 232 92 C218 74 232 60 250 66 C276 74 278 122 232 158 Z"
            fill="url(#heartG)" filter="url(#soft)" />
          <path d="M214 84 q10 -10 22 -3" stroke="#fff" strokeWidth="5" fill="none" strokeLinecap="round" opacity="0.65" />
          <ellipse cx="222" cy="80" rx="6" ry="4" fill="#fff" opacity="0.35" />
          {/* koszorúér-rajz */}
          <path d="M232 96 q-8 12 -4 26 M232 96 q8 12 4 26" stroke="#B71C1C" strokeWidth="2" fill="none" opacity="0.5" />
        </g>
        <style>{`@keyframes m6beat{0%,100%{transform:scale(1)}22%{transform:scale(1.15)}40%{transform:scale(1)}}@keyframes m6ekg{from{stroke-dashoffset:760}to{stroke-dashoffset:0}}`}</style>
      </>),

    /* 7 — fekete teherautó "HALÁL", forgó kerekek, kipufogófüst, mozgó út */
    7: wrap(
      <>
        <linearGradient id="truckG" x1="0" y1="0" x2="0" y2="1">
          <stop offset="0%" stopColor="#37474F" /><stop offset="100%" stopColor="#1C262B" />
        </linearGradient>
        <linearGradient id="cabG" x1="0" y1="0" x2="0" y2="1">
          <stop offset="0%" stopColor="#546E7A" /><stop offset="100%" stopColor="#37474F" />
        </linearGradient>
      </>,
      <>
        <line x1="0" y1="168" x2="320" y2="168" stroke="#B0BEC5" strokeWidth="3" strokeDasharray="20 14"
          style={{ animation: "m7road 0.6s linear infinite" }} />
        <g style={{ animation: "m7truck 3.4s ease-in-out infinite" }}>
          <rect x="58" y="82" width="146" height="64" rx="7" fill="url(#truckG)" filter="url(#soft)" />
          <rect x="58" y="82" width="146" height="14" rx="7" fill="#fff" opacity="0.08" />
          <path d="M204 100 h34 a8 8 0 0 1 8 8 v38 h-50 z" fill="url(#cabG)" />
          <path d="M212 106 h24 a4 4 0 0 1 4 4 v14 h-32 v-14 a4 4 0 0 1 4 -4z" fill="#B3E5FC" opacity="0.85" />
          <polygon points="214,108 236,108 232,122 214,122" fill="#fff" opacity="0.25" />
          <text x="128" y="126" textAnchor="middle" fontSize="28" fontWeight="900" fill="#fff" letterSpacing="3" fontFamily="sans-serif">HALÁL</text>
          <rect x="242" y="138" width="8" height="6" rx="2" fill="#FDD835" />
          {[96, 224].map((cx) => (
            <g key={cx}>
              <circle cx={cx} cy="154" r="16" fill="#111" />
              <circle cx={cx} cy="154" r="16" fill="none" stroke="#2A2A2A" strokeWidth="3" />
              <g style={{ transformOrigin: `${cx}px 154px`, animation: "m7wheel 0.45s linear infinite" }}>
                <circle cx={cx} cy="154" r="6.5" fill="#607D8B" />
                {[0, 72, 144, 216, 288].map((a) => (
                  <line key={a} x1={cx} y1="154" x2={cx + Math.cos(a * Math.PI / 180) * 14} y2={154 + Math.sin(a * Math.PI / 180) * 14} stroke="#455A64" strokeWidth="2.5" />
                ))}
              </g>
            </g>
          ))}
        ...
        {[0, 1, 2, 3].map((k) => (
          <circle key={k} cx={52 - k * 15} cy={96 - k * 5} r={5 + k * 2.5} fill="#90A4AE"
            style={{ animation: `m7smoke 1.8s ease-out ${k * 0.28}s infinite` }} />
        ))}
        <style>{`@keyframes m7truck{0%,100%{transform:translateX(-16px)}50%{transform:translateX(16px)}}@keyframes m7wheel{to{transform:rotate(360deg)}}@keyframes m7road{to{transform:translateX(-34px)}}@keyframes m7smoke{0%{opacity:.55;transform:translate(0,0) scale(.5)}100%{opacity:0;transform:translate(-24px,-16px) scale(1.6)}}`}</style>
      </>),

    /* 8 — síró futó, könnyek, sebességvonalak */
    8: wrap(null,
      <>
        {shadow(160, 186, 80)}
        {[74, 92, 110, 128].map((y, k) => (
          <line key={y} x1="58" y1={y} x2={112 - k * 10} y2={y} stroke="#B0BEC5" strokeWidth="3.5" strokeLinecap="round"
            style={{ animation: `m8speed 0.6s linear ${k * 0.08}s infinite` }} />
        ))}
        <g style={{ transformOrigin: "168px 118px", animation: "m8bob 0.5s ease-in-out infinite" }}>
          <circle cx="176" cy="54" r="23" fill="url(#skin)" />
          <circle cx="176" cy="54" r="23" fill="url(#skinHi)" />
          <path d="M154 48 a22 15 0 0 1 44 0 q-22 -7 -44 0z" fill="#4E342E" />
          <ellipse cx="169" cy="54" rx="2.6" ry="3.4" fill="#2B2118" /><ellipse cx="184" cy="54" rx="2.6" ry="3.4" fill="#2B2118" />
          {/* lefelé görbülő, síró száj */}
          <path d="M166 66 q10 -6 20 0" stroke="#8A3B3B" strokeWidth="2.6" fill="none" strokeLinecap="round" />
          {/* test */}
          <path d="M156 76 q20 -8 40 0 l-5 42 -30 0z" fill="#2F6FE0" filter="url(#soft)" />
          <path d="M176 78 v40" stroke="#1E4FA0" strokeWidth="1.5" opacity="0.4" />
          {/* karok */}
          <line x1="158" y1="88" x2="136" y2="108" stroke="url(#skin)" strokeWidth="8.5" strokeLinecap="round" />
          <line x1="192" y1="88" x2="212" y2="70" stroke="url(#skin)" strokeWidth="8.5" strokeLinecap="round" />
          {/* lábak */}
          <g style={{ transformOrigin: "170px 118px", animation: "m8legs 0.5s ease-in-out infinite" }}>
            <line x1="164" y1="118" x2="144" y2="152" stroke="#1E293B" strokeWidth="9.5" strokeLinecap="round" />
          </g>
          <line x1="178" y1="118" x2="198" y2="150" stroke="#1E293B" strokeWidth="9.5" strokeLinecap="round" />
          <ellipse cx="140" cy="154" rx="9" ry="4" fill="#0F172A" /><ellipse cx="202" cy="152" rx="9" ry="4" fill="#0F172A" />
        </g>
        {[[162, 60], [188, 60]].map(([cx, cy], k) => (
          <path key={k} d={`M${cx} ${cy} q-3 6 0 9 q3 -3 0 -9z`} fill="#4FC3F7"
            style={{ animation: `m8cry 1s ease-in ${k * 0.35}s infinite` }} />
        ))}
        <style>{`@keyframes m8bob{0%,100%{transform:rotate(-3deg)}50%{transform:rotate(3deg)}}@keyframes m8legs{0%,100%{transform:rotate(-20deg)}50%{transform:rotate(20deg)}}@keyframes m8cry{0%{opacity:1;transform:translateY(0)}100%{opacity:0;transform:translateY(26px)}}@keyframes m8speed{0%{opacity:0;transform:translateX(12px)}50%{opacity:1}100%{opacity:0;transform:translateX(-12px)}}`}</style>
      </>),

    /* 9 — omladozó szovjet laktanya, hulló téglák, por */
    9: wrap(
      <linearGradient id="wallG" x1="0" y1="0" x2="0" y2="1">
        <stop offset="0%" stopColor="#B0AFAD" /><stop offset="100%" stopColor="#8A8987" />
      </linearGradient>,
      <>
        <rect x="0" y="166" width="320" height="34" fill="#8D6E63" opacity="0.25" />
        <polygon points="62,60 160,30 258,60 258,70 62,70z" fill="#5D4037" />
        <polygon points="110,44 160,30 190,38 130,50z" fill="#4E342E" opacity="0.6" />
        <rect x="66" y="66" width="188" height="102" fill="url(#wallG)" />
        <rect x="66" y="66" width="188" height="102" fill="none" stroke="#6D6C6A" strokeWidth="2" />
        {/* téglák */}
        {Array.from({ length: 5 }).map((_, r) => Array.from({ length: 9 }).map((_, c) => (
          <rect key={`${r}-${c}`} x={72 + c * 20 + (r % 2) * 4} y={72 + r * 18} width="17" height="14" rx="1" fill="#7E8C93"
            stroke="#6B7880" strokeWidth="0.8"
            style={{ animation: `m9fall 2.6s ease-in ${((r * 9 + c) % 11) * 0.16}s infinite`, transformOrigin: "center" }} />
        )))}
        {/* törött ablakok */}
        {[[96, 118], [206, 118]].map(([x, y], k) => (
          <g key={k}>
            <rect x={x} y={y} width="26" height="34" fill="#263238" />
            <line x1={x} y1={y} x2={x + 26} y2={y + 34} stroke="#78909C" strokeWidth="1.4" />
            <line x1={x + 26} y1={y} x2={x} y2={y + 34} stroke="#78909C" strokeWidth="1.4" />
            <line x1={x + 13} y1={y} x2={x + 13} y2={y + 34} stroke="#455A64" strokeWidth="2" />
          </g>
        ))}
        {/* csillag */}
        <path d="M160 88 l5 12 13 0 -10.5 8 4 13 -11.5 -8 -11.5 8 4 -13 -10.5 -8 13 0z" fill="#C62828" opacity="0.6" />
        {/* porfelhő */}
        {[90, 160, 230].map((x, k) => (
          <circle key={x} cx={x} cy="164" r="8" fill="#BCAAA4"
            style={{ animation: `m9dust 2.2s ease-out ${k * 0.5}s infinite` }} />
        ))}
        <style>{`@keyframes m9fall{0%,72%{opacity:1;transform:translateY(0) rotate(0)}86%{opacity:.35;transform:translateY(7px) rotate(7deg)}100%{opacity:1;transform:translateY(0)}}@keyframes m9dust{0%{opacity:0;transform:scale(.4)}40%{opacity:.5}100%{opacity:0;transform:translateY(-16px) scale(1.5)}}`}</style>
      </>),

    /* 10 — kilövő koleszterin-görbe rácson, LDL zászló, pontok */
    10: wrap(
      <linearGradient id="ldlG" x1="0" y1="1" x2="1" y2="0">
        <stop offset="0%" stopColor="#FFB74D" /><stop offset="100%" stopColor="#E53935" />
      </linearGradient>,
      <>
        {[130, 100, 70, 40].map((y) => <line key={y} x1="38" y1={y} x2="300" y2={y} stroke="#E0E0E0" strokeWidth="1.4" strokeDasharray="3 5" />)}
        <line x1="38" y1="164" x2="304" y2="164" stroke="#90A4AE" strokeWidth="3" />
        <line x1="38" y1="164" x2="38" y2="28" stroke="#90A4AE" strokeWidth="3" />
        <polygon points="38,164 98,150 152,116 208,66 292,26 292,164"
          fill="url(#ldlG)" opacity="0.14" />
        <polyline points="38,150 98,138 152,110 208,64 292,26"
          fill="none" stroke="url(#ldlG)" strokeWidth="5" strokeLinecap="round" strokeLinejoin="round"
          style={{ strokeDasharray: 360, animation: "m10climb 2.4s ease-out infinite" }} />
        {[[98, 138], [152, 110], [208, 64], [292, 26]].map(([cx, cy], k) => (
          <g key={k}>
            <circle cx={cx} cy={cy} r="5" fill="#fff" stroke="#E53935" strokeWidth="3" />
          </g>
        ))}
        <g style={{ transformOrigin: "292px 26px", animation: "m10flag 1.8s ease-in-out infinite" }}>
          <line x1="292" y1="26" x2="292" y2="4" stroke="#455A64" strokeWidth="2.5" />
          <path d="M292 4 l24 6 -24 7z" fill="#E53935" />
          <text x="292" y="13" fontSize="8" fontWeight="900" fill="#fff">LDL</text>
        </g>
        <style>{`@keyframes m10climb{from{stroke-dashoffset:360}to{stroke-dashoffset:0}}@keyframes m10flag{0%,100%{transform:rotate(0)}50%{transform:rotate(-7deg)}}`}</style>
      </>),

    /* 11 — padlón hanyatt fekvő, kidolgozott figura (profil); kifogás-buborékok */
    11: wrap(
      <linearGradient id="shirt11" x1="0" y1="0" x2="0" y2="1">
        <stop offset="0%" stopColor="#3B82F6" /><stop offset="100%" stopColor="#1E4FA0" />
      </linearGradient>,
      <>
        {shadow(160, 172, 96)}
        <g filter="url(#soft)">
          {/* hátsó láb */}
          <path d="M150 138 q46 -6 74 -2 q7 1 7 8 q0 6 -7 7 q-30 3 -60 4 q-16 0 -14 -14z" fill="#173B7A" />
          <ellipse cx="228" cy="150" rx="10" ry="6" fill="#0F2A5C" />
          {/* behajlított közeli láb */}
          <path d="M150 140 q34 4 52 -10 q10 -8 4 -18 q-6 -8 -16 -2 q-22 14 -46 12z" fill="#1E293B" />
          <path d="M200 116 q10 -10 22 -8 q7 1 6 8 q-1 6 -8 7 q-12 1 -20 -7z" fill="#1E293B" />
          <ellipse cx="222" cy="118" rx="10" ry="7" fill="#0F172A" />
          {/* póló-törzs */}
          <path d="M104 138 q10 -22 40 -22 q34 0 44 8 q8 6 6 14 q-2 10 -14 12 q-40 6 -74 -2 q-10 -3 -8 -12z" fill="url(#shirt11)" />
          <ellipse cx="150" cy="132" rx="26" ry="15" fill="#2C63C8" opacity="0.55" />
          <path d="M120 120 q30 -6 58 2" stroke="#1B4FA0" strokeWidth="2" fill="none" opacity="0.4" />
          {/* hason nyugvó kar */}
          <path d="M156 128 q-12 12 -30 12 q-8 0 -8 -6 q0 -5 7 -6 q14 -1 23 -8z" fill="url(#skin)" />
          <ellipse cx="120" cy="134" rx="7" ry="6" fill="url(#skin)" />
          {/* nyak */}
          <path d="M104 128 q-8 2 -14 6 l4 12 q10 -4 16 -6z" fill="url(#skin)" />
          {/* fej profilban */}
          <g transform="translate(78 120)">
            <circle cx="0" cy="0" r="18" fill="url(#skin)" />
            <circle cx="0" cy="0" r="18" fill="url(#skinHi)" />
            <path d="M-18 -2 a18 18 0 0 1 30 -12 q-6 -4 -14 -3 q-12 2 -16 15z" fill="#4E342E" />
            <path d="M8 -14 q10 2 9 12 q-4 -8 -12 -8z" fill="#4E342E" />
            <ellipse cx="2" cy="2" rx="4" ry="5" fill="#EBB884" />
            <path d="M-12 -1 q4 -3 8 0" stroke="#2B2118" strokeWidth="1.8" fill="none" strokeLinecap="round" />
            <path d="M-17 2 q-4 2 -1 5 l4 -1z" fill="#EBB884" />
            <ellipse cx="-13" cy="9" rx="3.2" ry="4" fill="#8A3B3B" style={{ animation: "m11yawn 3s ease-in-out infinite" }} />
            <circle cx="-7" cy="6" r="3.5" fill="#F19B9B" opacity="0.4" />
          </g>
          {/* párna */}
          <ellipse cx="80" cy="140" rx="26" ry="10" fill="#CBD5E1" />
          <ellipse cx="80" cy="138" rx="22" ry="8" fill="#E2E8F0" />
        </g>
        {/* Zzz */}
        {[[100, 96, 13, 0], [116, 82, 17, 0.6], [134, 66, 22, 1.2]].map(([x, y, sz, d], k) => (
          <text key={k} x={x} y={y} fontSize={sz} fontWeight="900" fill="#94A3B8" fontFamily="sans-serif"
            style={{ animation: `m11z 3s ease-in-out ${d}s infinite` }}>Z</text>
        ))}
        {/* kifogás-buborékok */}
        {[["majd holnap", 232, 56, 0.3], ["fáj a térdem", 250, 116, 1.5]].map(([t, x, y, d], k) => (
          <g key={k} style={{ transformOrigin: `${x}px ${y}px`, animation: `m11rise 3.4s ease-in-out ${d}s infinite` }}>
            <ellipse cx={x} cy={y} rx="52" ry="22" fill="#fff" stroke="#CBD5E1" strokeWidth="2" filter="url(#soft)" />
            <ellipse cx={x - 30} cy={y + 18} rx="7" ry="5" fill="#fff" stroke="#CBD5E1" strokeWidth="2" />
            <text x={x} y={y + 4} textAnchor="middle" fontSize="12" fontWeight="700" fill="#64748B">{t}</text>
          </g>
        ))}
        <style>{`@keyframes m11rise{0%{opacity:0;transform:translateY(14px) scale(.9)}22%{opacity:1;transform:translateY(0) scale(1)}70%{opacity:1}86%{opacity:0;transform:scale(1.2)}100%{opacity:0}}@keyframes m11z{0%{opacity:0;transform:translateY(6px)}40%{opacity:1}100%{opacity:0;transform:translateY(-10px)}}@keyframes m11yawn{0%,78%,100%{transform:scaleY(.55)}88%{transform:scaleY(1.2)}}`}</style>
      </>),

    /* 12 — GPS kijelző pörgő iránytűvel, villódzó útvonallal */
    12: wrap(
      <linearGradient id="gpsG" x1="0" y1="0" x2="0" y2="1">
        <stop offset="0%" stopColor="#22303F" /><stop offset="100%" stopColor="#141C26" />
      </linearGradient>,
      <>
        <rect x="82" y="24" width="156" height="150" rx="18" fill="url(#gpsG)" filter="url(#soft)" />
        <rect x="82" y="24" width="156" height="150" rx="18" fill="none" stroke="#37485C" strokeWidth="3" />
        {[52, 78, 104, 130].map((y) => <line key={y} x1="92" y1={y} x2="228" y2={y} stroke="#2C3A4C" strokeWidth="2" />)}
        {[116, 148, 180, 212].map((x) => <line key={x} x1={x} y1="38" x2={x} y2="140" stroke="#2C3A4C" strokeWidth="2" />)}
        <polyline points="106,140 128,100 172,100 172,58 214,58" fill="none" stroke="#3BC9DB" strokeWidth="5"
          strokeLinecap="round" strokeLinejoin="round" style={{ animation: "m12blink 0.9s steps(2) infinite" }} />
        <circle cx="106" cy="140" r="5" fill="#3BC9DB" />
        <g style={{ transformOrigin: "172px 88px", animation: "m12spin 2s linear infinite" }}>
          <circle cx="172" cy="88" r="20" fill="#0F172A" stroke="#3BC9DB" strokeWidth="2" />
          <polygon points="172,72 179,90 172,86 165,90" fill="#E53935" />
          <polygon points="172,104 179,86 172,90 165,86" fill="#ECEFF1" />
          <circle cx="172" cy="88" r="3" fill="#3BC9DB" />
        </g>
        <rect x="92" y="148" width="136" height="20" rx="7" fill="#0B121C" />
        <circle cx="104" cy="158" r="4" fill="#FF5252" style={{ animation: "m12blink 0.9s steps(2) infinite" }} />
        <text x="120" y="162" fontSize="12" fontWeight="900" fill="#FF5252" fontFamily="sans-serif">Újratervezés…</text>
        <style>{`@keyframes m12spin{to{transform:rotate(360deg)}}@keyframes m12blink{0%{opacity:1}50%{opacity:.3}}`}</style>
      </>),

    /* 13 — önbecsülés-tartály lassan lecsorog, csöppen */
    13: wrap(
      <linearGradient id="liqG" x1="0" y1="0" x2="0" y2="1">
        <stop offset="0%" stopColor="#5B8DEF" /><stop offset="100%" stopColor="#1E4FA0" />
      </linearGradient>,
      <>
        <text x="160" y="26" textAnchor="middle" fontSize="13" fontWeight="900" fill="#64748B" letterSpacing="1">ÖNBECSÜLÉS</text>
        <rect x="126" y="38" width="68" height="132" rx="34" fill="#F4F6F8" stroke="#B0BEC5" strokeWidth="4" />
        <clipPath id="tank13"><rect x="130" y="42" width="60" height="124" rx="30" /></clipPath>
        <g clipPath="url(#tank13)">
          <rect x="130" y="118" width="60" height="70" fill="url(#liqG)" style={{ animation: "m13drain 3.4s ease-in-out infinite" }} />
          <ellipse cx="160" cy="118" rx="30" ry="6" fill="#7BA5F5" style={{ animation: "m13drain 3.4s ease-in-out infinite" }} />
          {/* buborékok */}
          {[[146, 150], [172, 140], [158, 158]].map(([x, y], k) => (
            <circle key={k} cx={x} cy={y} r="3" fill="#fff" opacity="0.4"
              style={{ animation: `m13bub 2s ease-in ${k * 0.5}s infinite` }} />
          ))}
        </g>
        <ellipse cx="146" cy="60" rx="8" ry="20" fill="#fff" opacity="0.35" />
        {[64, 92, 120, 148].map((y) => <line key={y} x1="194" y1={y} x2="206" y2={y} stroke="#90A4AE" strokeWidth="2" />)}
        <circle cx="160" cy="176" r="4.5" fill="#2F6FE0" style={{ animation: "m13drip 1.4s ease-in infinite" }} />
        <style>{`@keyframes m13drain{0%{transform:translateY(0)}50%{transform:translateY(48px)}100%{transform:translateY(0)}}@keyframes m13drip{0%{opacity:0;transform:translateY(0)}30%{opacity:1}100%{opacity:0;transform:translateY(18px)}}@keyframes m13bub{0%{opacity:0;transform:translateY(0)}40%{opacity:.6}100%{opacity:0;transform:translateY(-30px)}}`}</style>
      </>),

    /* 14 — kanapén oldalt alvó, felismerhető figura (profil); gravitációs mező */
    14: wrap(
      <>
        <radialGradient id="fieldG" cx="0.5" cy="0.5" r="0.6">
          <stop offset="0%" stopColor="#B39DDB" stopOpacity="0.5" /><stop offset="100%" stopColor="#B39DDB" stopOpacity="0" />
        </radialGradient>
        <linearGradient id="couch2" x1="0" y1="0" x2="0" y2="1">
          <stop offset="0%" stopColor="#9D8DD1" /><stop offset="100%" stopColor="#7E6BB0" />
        </linearGradient>
      ...
      <>
        <ellipse cx="160" cy="152" rx="118" ry="32" fill="url(#fieldG)"
          style={{ transformOrigin: "160px 152px", animation: "m14grav 2.2s ease-in-out infinite" }} />
        {/* lefelé szívó nyilak */}
        {[122, 160, 198].map((x, k) => (
          <g key={x} style={{ animation: `m14pull 1.5s ease-in ${k * 0.2}s infinite` }}>
            <line x1={x} y1="30" x2={x} y2="60" stroke="#7E57C2" strokeWidth="3.5" strokeLinecap="round" />
            <polygon points={`${x - 6},56 ${x + 6},56 ${x},68`} fill="#7E57C2" />
          </g>
        ))}
        {/* kanapé */}
        <rect x="60" y="136" width="200" height="42" rx="14" fill="url(#couch2)" filter="url(#soft)" />
        <rect x="52" y="98" width="30" height="80" rx="12" fill="#8877C2" />
        <rect x="238" y="98" width="30" height="80" rx="12" fill="#8877C2" />
        <rect x="84" y="124" width="152" height="18" rx="9" fill="#A99AD8" />

        {/* --- ALAK: oldalt fekve, profilból, tükörképe a 11-esnek --- */}
        <g filter="url(#soft)">
          {/* felső, nyújtott láb (távolabbi, sötétebb nadrág) */}
          <path d="M150 150 q40 -2 66 4 q7 1 6 8 q-1 6 -8 6 q-28 -2 -56 -2 q-14 0 -8 -16z" fill="#15315F" />
          <ellipse cx="220" cy="162" rx="10" ry="6" fill="#0E223F" />
          {/* alsó, kissé behajlított láb */}
          <path d="M150 152 q38 4 60 -2 q7 -2 8 5 q1 6 -6 8 q-30 4 -60 3 q-14 0 -2 -14z" fill="#1E3A6E" />
          <ellipse cx="216" cy="164" rx="10" ry="6" fill="#12264A" />
          {/* felsőtest — pulóver */}
          <path d="M96 148 q10 -20 38 -20 q30 0 42 8 q9 6 7 14 q-2 9 -13 11 q-38 6 -70 -1 q-11 -3 -9 -13z" fill="url(#hoodie)" />
          {/* váll-gyűrődés */}
          <path d="M116 132 q30 -4 54 6" stroke="#173B7A" strokeWidth="2" fill="none" opacity="0.45" />
          {/* has-domborulat */}
          <ellipse cx="140" cy="142" rx="24" ry="13" fill="#2C63C8" opacity="0.45" />
          {/* párna alatti kar (pihen) */}
          <path d="M150 140 q-14 10 -32 8 q-8 -1 -7 -7 q1 -5 8 -5 q16 0 27 -6z" fill="url(#skin)" />
          <ellipse cx="112" cy="146" rx="7" ry="6" fill="url(#skin)" />
          {/* nyak */}
          <path d="M96 138 q-8 1 -14 4 l3 12 q10 -3 16 -5z" fill="url(#skin)" />
          {/* FEJ profil a párnán */}
          <g transform="translate(72 128)">
            ...
            <path d="M-17 2 q-4 2 -1 5 l4 -1z" fill="#EBB884" />
            {/* elégedett mosoly */}
            <path d="M-15 9 q4 3 9 1" stroke="#8A3B3B" strokeWidth="1.8" fill="none" strokeLinecap="round" />
            <circle cx="-8" cy="6" r="3.5" fill="#F19B9B" opacity="0.4" />
          </g>
          {/* párna */}
          <ellipse cx="74" cy="148" rx="26" ry="10" fill="#CBD5E1" />
          <ellipse cx="74" cy="146" rx="22" ry="8" fill="#E2E8F0" />
        </g>
        {/* Zzz */}
        {[[96, 108, 12, 0], [110, 96, 16, 0.6], [126, 82, 20, 1.2]].map(([x, y, sz, d], k) => (
          <text key={k} x={x} y={y} fontSize={sz} fontWeight="900" fill="#94A3B8" fontFamily="sans-serif"
            style={{ animation: `m14z 3s ease-in-out ${d}s infinite` }}>Z</text>
        ))}
        <style>{`@keyframes m14grav{0%,100%{transform:scale(1);opacity:.9}50%{transform:scale(1.12);opacity:1}}@keyframes m14pull{0%{opacity:0;transform:translateY(-8px)}50%{opacity:1}100%{opacity:0;transform:translateY(10px)}}@keyframes m14z{0%{opacity:0;transform:translateY(6px)}40%{opacity:1}100%{opacity:0;transform:translateY(-10px)}}`}</style>
      </>),

    /* 15 — menekülő futó egy fenyegető kaszás-árnyék elől */
    15: wrap(null,
      <>
        {shadow(120, 186, 70)}
        {[70, 90, 110].map((y, k) => (
          <line key={y} x1="30" y1={y} x2={80 - k * 10} y2={y} stroke="#B0BEC5" strokeWidth="3.5" strokeLinecap="round"
            style={{ animation: `m15speed 0.5s linear ${k * 0.08}s infinite` }} />
        ))}
        {/* fenyegető kaszás-árnyék */}
        <g style={{ transformOrigin: "236px 110px", animation: "m15loom 1.8s ease-in-out infinite" }} opacity="0.82">
          <path d="M236 60 q-22 4 -22 34 l0 62 q0 6 6 6 l32 0 q6 0 6 -6 l0 -62 q0 -30 -22 -34z" fill="#2A2A33" />
          <path d="M236 56 q-16 0 -16 18 q0 14 16 16 q16 -2 16 -16 q0 -18 -16 -18z" fill="#1C1C24" />
          <ellipse cx="230" cy="74" rx="2.4" ry="3" fill="#FF5252" />
          <ellipse cx="242" cy="74" rx="2.4" ry="3" fill="#FF5252" />
          <line x1="262" y1="40" x2="262" y2="150" stroke="#3A2E22" strokeWidth="4" strokeLinecap="round" />
          <path d="M262 42 q26 -6 34 14 q-20 -8 -34 2z" fill="#B0BEC5" />
          <path d="M262 42 q26 -6 34 14" fill="none" stroke="#eceff1" strokeWidth="1.5" opacity="0.7" />
        </g>
        {/* menekülő futó */}
        <g style={{ transformOrigin: "120px 116px", animation: "m15bob 0.42s ease-in-out infinite" }}>
          <circle cx="128" cy="52" r="22" fill="url(#skin)" />
          <circle cx="128" cy="52" r="22" fill="url(#skinHi)" />
          <path d="M107 46 a22 15 0 0 1 42 0 q-21 -7 -42 0z" fill="#4E342E" />
          <ellipse cx="118" cy="52" rx="3" ry="3.6" fill="#fff" /><ellipse cx="119" cy="53" rx="1.5" ry="2" fill="#2B2118" />
          <ellipse cx="131" cy="52" rx="3" ry="3.6" fill="#fff" /><ellipse cx="132" cy="53" rx="1.5" ry="2" fill="#2B2118" />
          <ellipse cx="128" cy="63" rx="3" ry="4" fill="#8A3B3B" />
          <path d="M110 46 q-3 5 0 8 q3 -3 0 -8z" fill="#4FC3F7" style={{ animation: "m15sweat 0.9s ease-out infinite" }} />
          <path d="M108 74 q20 -8 40 0 l-5 42 -30 0z" fill="#2F6FE0" filter="url(#soft)" />
          <line x1="110" y1="86" x2="88" y2="102" stroke="url(#skin)" strokeWidth="8.5" strokeLinecap="round" />
          <line x1="146" y1="86" x2="166" y2="70" stroke="url(#skin)" strokeWidth="8.5" strokeLinecap="round" />
          <g style={{ transformOrigin: "122px 116px", animation: "m15legs 0.42s ease-in-out infinite" }}>
            <line x1="116" y1="116" x2="96" y2="150" stroke="#1E293B" strokeWidth="9.5" strokeLinecap="round" />
          </g>
          <line x1="130" y1="116" x2="150" y2="150" stroke="#1E293B" strokeWidth="9.5" strokeLinecap="round" />
          <ellipse cx="92" cy="152" rx="9" ry="4" fill="#0F172A" /><ellipse cx="154" cy="152" rx="9" ry="4" fill="#0F172A" />
        </g>
        <style>{`@keyframes m15bob{0%,100%{transform:rotate(-3deg)}50%{transform:rotate(3deg)}}@keyframes m15legs{0%,100%{transform:rotate(-22deg)}50%{transform:rotate(22deg)}}@keyframes m15speed{0%{opacity:0;transform:translateX(12px)}50%{opacity:1}100%{opacity:0;transform:translateX(-12px)}}@keyframes m15loom{0%,100%{transform:scale(1) translateX(0)}50%{transform:scale(1.05) translateX(-6px)}}@keyframes m15sweat{0%{opacity:0;transform:translateY(0)}30%{opacity:1}100%{opacity:0;transform:translateY(16px)}}`}</style>
      </>),
  };
...
function KingMotivation({ accent, queen }) {
  const [open, setOpen] = useState(false);
  const [i, setI] = useState(() => Math.floor(Math.random() * MOTIVATIONS.length));

  const roll = () => setI(Math.floor(Math.random() * MOTIVATIONS.length));
  const openWith = () => { roll(); setOpen(true); };
    ...
    <>
      ...
      <button onClick={openWith}
        ...
        <span className="relative flex h-12 w-12 items-center justify-center">
          {/* lobogó láng */}
          <svg width="46" height="52" viewBox="0 0 46 52" className="absolute -z-0">
            <defs>
              <linearGradient id="flameG" x1="0" y1="1" x2="0" y2="0">
                <stop offset="0%" stopColor="#FF3D00" /><stop offset="55%" stopColor="#FF9100" /><stop offset="100%" stopColor="#FFEA00" />
              </linearGradient>
            </defs>
            <path d="M23 52 C6 44 8 26 18 18 C18 26 24 26 24 18 C24 10 20 6 26 0 C24 12 38 16 38 32 C38 44 32 49 23 52 Z"
              fill="url(#flameG)" style={{ transformOrigin: "23px 46px", animation: "flame 0.5s ease-in-out infinite alternate" }} />
          </svg>
          {/* ököl emoji */}
          <span className="relative z-10 text-2xl" style={{ animation: "punch 0.9s ease-in-out infinite" }}>👊</span>
        </span>
        <span className="max-w-[70px] text-center text-[10px] font-black leading-tight"
          style={{ color: accent }}>
          Kérj motivációt a királytól
        </span>
        <style>{`
          @keyframes flame { 0%{transform:scaleY(1) scaleX(1)} 100%{transform:scaleY(1.12) scaleX(.92)} }
          @keyframes punch { 0%,100%{transform:translateY(0) rotate(0)} 50%{transform:translateY(-3px) rotate(-8deg)} }
        `}</style>
      </button>

      {open && (
        <div className="fixed inset-0 z-[78] flex items-center justify-center px-5"
          style={{ background: "rgba(15,23,42,.62)" }} onClick={() => setOpen(false)}>
          <Card className="w-full max-w-sm overflow-hidden p-0" onClick={(e) => e.stopPropagation()}
            style={{ boxShadow: "0 24px 60px rgba(0,0,0,.4)" }}>
            <div style={{ background: "#EEF4FA" }}>
              <MotivArt i={MOTIVATIONS[i].art} />
            </div>
            <div className="p-6 text-center">
              <p className="text-xs font-black uppercase tracking-widest"
                style={{ color: queen ? C.pink : C.purple }}>
                Üzenet a {queen ? "királynőtől" : "királytól"}
              </p>
              <p className="mt-2 text-base font-bold leading-relaxed" style={{ color: C.ink }}>
                {MOTIVATIONS[i].text}
              </p>
              <div className="mt-5 flex gap-2">
                <Button className="flex-1" onClick={roll}>
                  <Flame size={16} /> Még egyet!
                </Button>
                <Button variant="ghost" onClick={() => setOpen(false)}>Elég</Button>
              </div>
            </div>
          </Card>
        </div>
  ...
  return (
    <Card className="relative overflow-hidden p-4" style={{ border: "none", background: bg }}>
      ...
      <div className="relative flex items-center gap-3">
        ...
        {!empty && <KingMotivation accent={accent} queen={queen} />}
      </div>
    </Card>
  );
}
...
function FeedItem({ item, likes, meId, onLike }) {
  ...
  return (
    <div className="flex gap-3 py-3.5" style={{ borderTop: `1px solid ${C.line}` }}>
        ...
        {item.type === "weight" && (
          <div className="mt-1 flex flex-wrap items-center gap-2">
            ...
            {item.delta != null && item.delta !== 0 ? (
              ...
              <Pill tone="slate">első mérés</Pill>
            ) : null}
            {u.hide_weight && u.id === meId && (
              <Pill tone="blue"><EyeOff size={11} /> rejtve</Pill>
            )}
          </div>
        )}
      ...
      {!(item.type === "weight" && u.hide_weight) && (
...
/* ======================= Napi motivációs popup ========================== */
function DailyNudge({ me, remaining, onClose }) {
  return (
    <div className="fixed inset-0 z-[75] flex items-center justify-center px-5"
      style={{ background: "rgba(15,23,42,.6)" }} onClick={onClose}>
      <Card className="w-full max-w-sm overflow-hidden p-0" onClick={(e) => e.stopPropagation()}
        style={{ boxShadow: "0 24px 60px rgba(0,0,0,.35)" }}>
        <style>{`
          @keyframes nudgeDance {
            0%,100% { transform: rotate(-5deg) translateY(0); }
            25%     { transform: rotate(4deg) translateY(-4px); }
            50%     { transform: rotate(-3deg) translateY(0); }
            75%     { transform: rotate(5deg) translateY(-3px); }
          }
          @keyframes nudgeJiggle {
            0%,100% { transform: scaleX(1); }
            50%     { transform: scaleX(1.05); }
          }
          @keyframes nudgeSweat {
            0%   { opacity: 0; transform: translate(0,0) scale(.5); }
            30%  { opacity: 1; }
            100% { opacity: 0; transform: translate(var(--sx), 22px) scale(1); }
          }
        `}</style>

        <div className="flex items-center justify-center py-6"
          style={{ background: `linear-gradient(135deg, ${C.cyan}22, ${C.blue}18)` }}>
          <svg width="150" height="170" viewBox="0 0 150 170"
            style={{ animation: "nudgeDance 1.1s ease-in-out infinite", transformOrigin: "75px 150px" }}>
            <defs>
              <radialGradient id="skinG" cx="0.4" cy="0.35">
                <stop offset="0%" stopColor="#FAD8B0" /><stop offset="100%" stopColor="#E8B584" />
              </radialGradient>
            </defs>

            {/* izzadságcseppek */}
            {[["-18px", 40], ["16px", 44]].map(([sx, cx], i) => (
              <circle key={i} cx={75 + (i ? 26 : -26)} cy="46" r="3" fill={C.cyan}
                style={{ "--sx": sx, animation: `nudgeSweat 1.4s ease-out ${i * 0.5}s infinite` }} />
            ))}

            {/* láb */}
            <path d="M58 132 q-4 18 -2 30 l10 0 q1 -14 4 -26 z" fill="url(#skinG)" />
            <path d="M92 132 q4 18 2 30 l-10 0 q-1 -14 -4 -26 z" fill="url(#skinG)" />
            <ellipse cx="60" cy="164" rx="9" ry="4" fill="#3B3B3B" />
            <ellipse cx="90" cy="164" rx="9" ry="4" fill="#3B3B3B" />

            {/* pocakos test */}
            <ellipse cx="75" cy="104" rx="40" ry="36" fill="url(#skinG)"
              style={{ animation: "nudgeJiggle 1.1s ease-in-out infinite", transformOrigin: "75px 104px" }} />
            {/* köldök */}
            <ellipse cx="75" cy="112" rx="2.5" ry="4" fill="#C98F5E" opacity="0.6" />

            {/* cenzúra-csík (fekete pixel-sáv) */}
            <g>
              <rect x="55" y="118" width="40" height="18" rx="3" fill="#111" />
              {Array.from({ length: 5 }).map((_, i) => (
                <rect key={i} x={57 + i * 7.5} y="120" width="6" height="6" fill="#333" />
              ))}
              {Array.from({ length: 5 }).map((_, i) => (
                <rect key={`b${i}`} x={60 + i * 7.5} y="128" width="6" height="6" fill="#000" />
              ))}
              <text x="75" y="132" textAnchor="middle" fontSize="7" fontWeight="900" fill="#fff" opacity="0.5">CENZÚRA</text>
            </g>

            {/* karok fel — táncol */}
            <path d="M40 92 q-20 -10 -26 -28 l7 -4 q8 14 24 22 z" fill="url(#skinG)"
              style={{ animation: "nudgeDance 1.1s ease-in-out infinite", transformOrigin: "40px 92px" }} />
            <path d="M110 92 q20 -10 26 -28 l-7 -4 q-8 14 -24 22 z" fill="url(#skinG)"
              style={{ animation: "nudgeDance 1.3s ease-in-out infinite reverse", transformOrigin: "110px 92px" }} />

            {/* mellkas */}
            <circle cx="62" cy="82" r="7" fill="#E0A87A" opacity="0.5" />
            <circle cx="88" cy="82" r="7" fill="#E0A87A" opacity="0.5" />

            {/* fej */}
            <circle cx="75" cy="52" r="22" fill="url(#skinG)" />
            <path d="M56 44 a19 12 0 0 1 38 0 q-19 -6 -38 0 z" fill="#5B3A1E" opacity="0.35" />
            <circle cx="68" cy="52" r="2.4" fill="#2B2118" />
            <circle cx="82" cy="52" r="2.4" fill="#2B2118" />
            <path d="M67 61 q8 6 16 0" stroke="#B5715A" strokeWidth="2" fill="none" strokeLinecap="round" />
            <circle cx="61" cy="58" r="3.5" fill="#F19B9B" opacity="0.5" />
            <circle cx="89" cy="58" r="3.5" fill="#F19B9B" opacity="0.5" />
            {/* tokák */}
            <path d="M60 70 q15 12 30 0 q-15 6 -30 0 z" fill="#E8B584" />
          </svg>
        </div>

        <div className="p-6 text-center">
          <p className="text-lg font-black leading-snug" style={{ color: C.ink }}>
            Szia {me.name.split(" ").slice(-1)[0] || me.name}!
          </p>
          {remaining > 0 ? (
            <p className="mt-2 text-sm leading-relaxed" style={{ color: C.slate }}>
              A héten még <b className="tabular-nums" style={{ color: C.blue }}>{remaining.toLocaleString("hu-HU")}</b> kalóriát
              kell elégetned, és akkor nem leszel ilyen kövér. Hajrá!
            </p>
          ) : (
            <p className="mt-2 text-sm leading-relaxed" style={{ color: C.slate }}>
              A heti célod már megvan — nem leszel ilyen kövér. Ma már csak a dicsőségért nyomod. Hajrá!
            </p>
          )}
          <Button className="mt-5 w-full py-3.5 text-base" onClick={onClose}>Nyomjuk!</Button>
        </div>
      </Card>
    </div>
  );
...
export default function App() {
  ...
  /* ---- Napi motivációs popup: naponta egyszer, csoportnézetben ---- */
  useEffect(() => {
    if (!me || !group || notStarted(group)) return;
    const key = `pc-nudge-${me.id}-${TODAY}`;
    try {
      if (!sessionStorage.getItem(key)) {
        setNudge(true);
        sessionStorage.setItem(key, "1");
      }
    } catch { /* privát mód: kihagyjuk */ }
  }, [me, group]); // eslint-disable-line
  ...
  return (
    <div className="min-h-screen pb-24 md:pb-8" style={{ background: "#fff" }}>
      ...
      {nudge && stats && (
        <DailyNudge me={me}
          remaining={Math.max(stats.goal - stats.weeks[weekIndexNow(group.start_date)].total, 0)}
