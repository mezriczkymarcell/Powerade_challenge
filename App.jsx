  ...
  return (
    <div className="flex items-stretch gap-3 py-4" style={{ borderTop: `1px solid ${C.line}` }}>
      ...
      <div className="flex min-w-0 flex-1 flex-col justify-center">
        ...
        {/* másodlagos: kis avatar + név + idő */}
        <div className="mt-1.5 flex items-center gap-1.5">
          <Avatar user={u} size={18} badge={false} />
          <span className="truncate text-xs font-semibold" style={{ color: C.slate }}>{u.name}</span>
          <span className="text-xs" style={{ color: C.line }}>·</span>
          <span className="shrink-0 text-xs" style={{ color: C.slate }}>{relTime(item.ts)}</span>
        </div>
      </div>

      {/* NAGY, feltűnő szív jobbra */}
      <button onClick={() => onLike(item)}
        className="flex shrink-0 flex-col items-center justify-center gap-0.5 self-center rounded-2xl px-3 py-2 transition active:scale-90"
        style={{ background: mine ? `${C.danger}12` : C.soft }}>
        <Heart size={26} strokeWidth={2.5}
          style={{ color: mine ? C.danger : C.slate, fill: mine ? C.danger : "transparent" }} />
        <span className="text-sm font-black tabular-nums" style={{ color: mine ? C.danger : C.slate }}>
          {count}
        </span>
      </button>
    </div>
