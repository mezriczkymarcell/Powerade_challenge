...
function TrackerView({ me, group, sports, onAddSport, onSave, onDelete, toast, initialDate }) {
  ...
  const clearForm = () => {
    setKcal(""); setFile(null); setPreview(null);
    ...
    <div className="space-y-5">
      ...
      <Card className="p-6">
        ...
        {items.length === 0 ? (
          ...
          <div className="space-y-2">
            {items.map((it) => (
              <div key={it.id} className="flex items-center gap-3 rounded-xl px-3 py-3" style={{ border: `1px solid ${C.line}` }}>
                <div className="flex h-9 w-9 items-center justify-center rounded-lg" style={{ background: `${C.blue}12` }}>
                ...
                <span className="text-sm font-black tabular-nums" style={{ color: C.blue }}>{it.kcal}</span>
                <button onClick={() => onDelete(it.id)} className="rounded-lg p-1.5" title="Törlés">
...
export default function App() {
  ...
  const addEntry = async (date, kcal, sport, file) => {
    let proof_url = null;
    if (file) {
    ...
    const { error } = await supabase.from("entries")
      .insert({ user_id: session.user.id, date, kcal, sport, proof_url });
