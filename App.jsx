...
import {
  ...
  LogOut, AlertTriangle, Clock, Camera, X, RefreshCw, Users, UserPlus, ArrowLeft, Crown
} from "lucide-react";

/* ============================================================================
   POWERADE CHALLENGE  ·  csoportos verzió
   ----------------------------------------------------------------------------
   - Minden csoport külön kihívás: saját kezdődátum, saját kassza, saját ranglista.
   - Csak a saját csoportjaid tagjait látod.
   - A napi bejegyzésed globális: minden csoportodban ugyanaz számít.
   - Heti cél: férfi 4000 kcal, nő 3000 kcal. Nem kell minden nap edzeni.
   - Ha vasárnap éjfélkor nincs meg a heti cél: 5000 Ft a közös kasszába.
   ========================================================================== */
...
const goalOf = (u) => (u?.gender === "female" ? GOAL_FEMALE : GOAL_MALE);
...
/** Az adott dátumot tartalmazó hét hétfője. */
...
/** Hány százalék van még hátra a 13 hétből (1 = tele palack, 0 = üres). */
...
const Field = ({ label, ...p }) => (
  <div>
    <label className="mb-1 block text-xs font-bold uppercase tracking-wider" style={{ color: C.slate }}>{label}</label>
...
/** pct: 1 = tele (kihívás eleje), 0 = üres (kihívás vége) */
function PoweradeBottle({ pct, daysLeft }) {
  const TOP = 58, BOT = 250;            // a folyadék tere a palackon belül
  const level = TOP + (BOT - TOP) * (1 - Math.max(0, Math.min(1, pct)));
  const empty = pct <= 0;
  ...
  return (
    <div className="flex flex-col items-center">
      <svg width="150" height="280" viewBox="0 0 150 280">
        <defs>
          {/* A palack belseje – ebbe vágjuk bele a folyadékot */}
          <clipPath id="bottleInner">
            <path d="M52 34 h46 v14 c0 7 4 11 9 16 c10 10 14 22 14 38 v128 c0 14 -10 24 -24 24 h-44 c-14 0 -24 -10 -24 -24 v-128 c0 -16 4 -28 14 -38 c5 -5 9 -9 9 -16 z" />
          </clipPath>
        ...
        <text x="75" y="152" textAnchor="middle" fontSize="13" fontWeight="900"
          fill={C.blueDeep} letterSpacing="0.5">POWERADE</text>
        <text x="75" y="169" textAnchor="middle" fontSize="10" fontWeight="700" fill={C.slate}>
          {empty ? "LEJÁRT" : `${daysLeft} NAP`}
        </text>
      </svg>

      <p className="mt-1 text-xs font-bold uppercase tracking-wider" style={{ color: C.slate }}>
        {empty ? "A kihívás véget ért" : `${Math.round(pct * 100)}% van hátra`}
      </p>
    </div>
  );
}
...
function AuthScreen() {
  ...
  const submit = async () => {
    ...
    try {
      ...
      setErr(e.message === "Invalid login credentials" ? "Hibás e-mail vagy jelszó." : e.message);
      ...
      <div className="mx-auto flex w-full max-w-md flex-col">
        <div className="mb-8">
          ...
          <p className="mt-3 text-sm leading-relaxed" style={{ color: C.slate }}>
            ...
            Akinek nem jön össze, 5000 Ft-tal tölti a közös kasszát.
          </p>
          <div className="mt-4 flex gap-2">
            {[["Férfi", GOAL_MALE], ["Nő", GOAL_FEMALE]].map(([l, g]) => (
              <div key={l} className="flex-1 rounded-xl px-3 py-2" style={{ background: C.soft }}>
                ...
                <p className="text-lg font-black tabular-nums" style={{ color: C.blue }}>
                  {g}<span className="text-xs font-bold" style={{ color: C.slate }}> kcal/hét</span>
                </p>
              </div>
            ))}
          </div>
        ...
        <div className="space-y-4">
          ...
          <Button onClick={submit} disabled={busy} className="w-full py-4 text-base">
            {busy ? "Egy pillanat…" : mode === "signup" ? "Regisztrálok" : "Belépés"}
...
function GroupsScreen({ me, groups, invites, onOpen, onCreate, onAccept, onSignOut, toast }) {
  ...
  const create = async () => {
    ...
    setBusy(true);
    const ok = await onCreate(gname.trim());
    setBusy(false);
    if (ok) { setGname(""); setCreating(false); toast("Csoport létrehozva. Hívd meg a többieket!", "ok"); }
  };
    ...
    <div className="min-h-screen px-5 py-8" style={{ background: "#fff" }}>
      <div className="mx-auto w-full max-w-2xl">
        <div className="mb-8 flex items-start justify-between">
          ...
          <button onClick={onSignOut} className="rounded-lg p-2"><LogOut size={18} style={{ color: C.slate }} /></button>
        </div>
              ...
              {invites.map((i) => (
                <Card key={i.id} className="flex items-center gap-3 p-4">
                  ...
                  <div className="flex-1">
                    <p className="text-sm font-bold" style={{ color: C.ink }}>{i.group_name}</p>
                    <p className="text-xs" style={{ color: C.slate }}>Meghívtak ebbe a csoportba</p>
                  </div>
                  <Button onClick={() => onAccept(i)}>Csatlakozom</Button>
                </Card>
              ))}
            </div>
          </div>
        ...
        <SectionTitle right={
          !creating && <Button variant="soft" onClick={() => setCreating(true)}><PlusCircle size={16} /> Új csoport</Button>
        }>
          Csoportjaim
        </SectionTitle>
        ...
        {creating && (
          <Card className="mb-4 p-5">
            <Field label="Csoport neve" value={gname} autoFocus placeholder="pl. Vértessomló brigád"
              onChange={(e) => setGname(e.target.value)} onKeyDown={(e) => e.key === "Enter" && create()} />
            <p className="mt-2 text-xs" style={{ color: C.slate }}>
              A kihívás a mostani hét hétfőjén indul, és 13 hétig tart. Te leszel a csoport gazdája: te hagyod jóvá a befizetéseket.
            </p>
            <div className="mt-4 flex gap-2">
        ...
        {groups.length === 0 && !creating ? (
          <Card className="p-10 text-center">
            ...
            <p className="mx-auto mt-1 max-w-xs text-sm" style={{ color: C.slate }}>
              Hozz létre egyet, vagy kérd meg a haverokat, hogy hívjanak meg erre az e-mail-címre: {me.email}
            </p>
          </Card>
        ) : (
          <div className="space-y-3">
            {groups.map((g) => {
              ...
              const started = g.start_date <= TODAY;
                  ...
                  <div className="flex items-start justify-between gap-3">
                    <div className="min-w-0 flex-1">
                      <div className="flex items-center gap-2">
                        <p className="truncate text-lg font-black" style={{ color: C.ink }}>{g.name}</p>
                        {g.owner_id === me.id && <Crown size={14} style={{ color: C.amber }} />}
                      </div>
                      <p className="mt-0.5 text-xs font-semibold" style={{ color: C.slate }}>
                        {started ? `${wk}. hét / ${TOTAL_WEEKS}` : `Indul: ${fmtHU(g.start_date)}`} · {g.members.length} tag
                      </p>
                      ...
                      <div className="mt-3 flex flex-wrap items-center gap-1">
                        ...
                        {g.members.length > 8 && (
                          <span className="text-xs font-bold" style={{ color: C.slate }}>+{g.members.length - 8}</span>
                        )}
                      </div>
                      ...
                      <div className="mt-4">
                        ...
                        <div className="mt-2 flex items-center justify-between">
                          <span className="text-xs font-semibold" style={{ color: C.slate }}>
                            Kassza: <b style={{ color: C.ink }}>{money(pool)}</b>
                          </span>
...
/* ============================ 3 hónapos naptár ============================= */
function ThreeMonthGrid({ entries, weeks, cw }) {
  return (
    <div className="space-y-2">
      {weeks.map((w) => (
        <div key={w.weekIndex} className="flex items-center gap-3">
          ...
          <div className="flex flex-1 gap-1">
            {w.days.map((d) => {
              ...
              return <div key={d} title={`${fmtHU(d)} — ${k} kcal`} className="h-6 flex-1 rounded-md"
...
/* =============================== Dashboard ================================ */
function DashboardView({ me, group, stats, pool, go }) {
  ...
  const urgent = missing > 0 && daysLeftWeek <= 2;
  const pct = remainingPct(group.start_date);
  const daysLeftAll = Math.max(TOTAL_DAYS - (diffDays(TODAY, group.start_date) + 1), 0);

  return (
    <div className="space-y-5">
      {/* A palack: a teljes kihívás visszaszámlálója */}
      <Card className="flex flex-col items-center p-6">
        <SectionTitle>A kihívás</SectionTitle>
        <PoweradeBottle pct={pct} daysLeft={daysLeftAll} />
      </Card>
      ...
      <Card className="p-6" style={{ background: `linear-gradient(135deg, ${C.blueDeep}, ${C.blue})`, border: "none" }}>
        <div className="flex items-start justify-between">
          <div>
            ...
            <p className="mt-1 text-sm font-medium text-white opacity-80">{group.name}</p>
          </div>
      ...
      <Card className="p-6">
        ...
        <ThreeMonthGrid entries={me.entries} weeks={stats.weeks} cw={cw} />
      </Card>
    </div>
  );
}
...
/* ================================ Tracker ================================= */
function TrackerView({ me, group, sports, onAddSport, onSave, toast }) {
      ...
      <Card className="p-6">
        ...
        <div className="space-y-4">
          <div>
            ...
            <div className="flex gap-2">
              {[[TODAY, "Ma"], [YESTERDAY, "Tegnap"]].map(([d, l]) => (
                <button key={d} onClick={() => setDate(d)} className="flex-1 rounded-xl py-3 text-sm font-bold transition"
                  style={{ background: date === d ? `${C.blue}0F` : C.soft, color: date === d ? C.blue : C.slate,
                    border: `1px solid ${date === d ? C.blue : C.line}` }}>
                  {l} · {fmtHU(d)}
                </button>
              ))}
            </div>
            <p className="mt-2 text-xs" style={{ color: C.slate }}>
              Nem kell minden nap edzeni — csak a heti összeg számít. Régebbi napra viszont nem lehet rögzíteni.
            </p>
          </div>
      ...
      <Card className="p-6">
        ...
        <div className="space-y-2">
          {weekDays(group.start_date, cw).map((d) => {
            ...
            const future = d > TODAY;
            const editable = d === TODAY || d === YESTERDAY;
            return (
              <button key={d} onClick={() => editable && setDate(d)} disabled={!editable}
    ...
    <div className="space-y-5">
      ...
      <Card className="overflow-hidden">
        <div className="flex items-center gap-3 px-4 py-3 text-xs font-bold uppercase tracking-wider"
          ...
          <span className="w-20 text-right">kcal</span><span className="w-24 text-right">Befizetve</span>
        </div>
        {rows.map((r, i) => (
          <div key={r.id} className="flex items-center gap-3 px-4 py-3"
            ...
            <div className="min-w-0 flex-1">
              ...
              <div className="mt-0.5 flex flex-wrap items-center gap-2">
                ...
                {r.debt > 0 ? <Pill tone="danger">Tartozás {money(r.debt)}</Pill> : <Pill tone="ok">Rendezve</Pill>}
      ...
      <p className="px-1 text-xs" style={{ color: C.slate }}>
        Mindenki ugyanabban a versenyben van, de a heti cél eltér: férfiaknak {GOAL_MALE}, nőknek {GOAL_FEMALE} kcal.
      </p>
    </div>
  );
}
...
/* ============================== Kassza / Tagok ============================ */
function WalletView({ me, group, stats, pool, onDeclare, onApprove, onInvite, toast }) {
  ...
  const invite = async () => {
    ...
    setBusy(false);
    if (ok) { setInvEmail(""); toast("Meghívó elküldve. A címzett a belépés után látja majd.", "ok"); }
  };
  ...
  return (
    <div className="space-y-5">
      ...
      <Card className="p-6">
        ...
        <p className="mt-2 text-xs" style={{ color: C.slate }}>
          A meghívott a regisztráció/belépés után a saját kezdőoldalán látja majd a csoportot.
        </p>
      </Card>
...
export default function App() {
  ...
  const load = useCallback(async () => {
    ...
    const myGroupIds = new Set((gm.data || []).filter((m) => m.user_id === session.user.id).map((m) => m.group_id));
    setGroups((gr.data || []).filter((g) => myGroupIds.has(g.id)).map((g) => ({
      ...g,
      members: (gm.data || []).filter((m) => m.group_id === g.id)
        .map((m) => users.find((u) => u.id === m.user_id)).filter(Boolean),
    })));

    const myEmail = (session.user.email || "").toLowerCase();
    setInvites((inv.data || [])
      .filter((i) => i.email.toLowerCase() === myEmail && !myGroupIds.has(i.group_id))
  ...
  const createGroup = async (name) => {
    const { data, error } = await supabase.from("groups")
      .insert({ name, owner_id: session.user.id, start_date: mondayOf(TODAY) }).select().single();
  ...
  const acceptInvite = async (inv) => {
    const { error } = await supabase.from("group_members")
      .insert({ group_id: inv.group_id, user_id: session.user.id });
  ...
  if (!group) return (
    <>
      <GroupsScreen me={me} groups={groups} invites={invites} toast={notify}
        onOpen={(id) => { setOpenId(id); setTab("dashboard"); }}
        onCreate={createGroup} onAccept={acceptInvite}
    ...
    <div className="min-h-screen pb-24 md:pb-8" style={{ background: "#fff" }}>
      <header className="sticky top-0 z-20 backdrop-blur"
        ...
        <div className="mx-auto flex max-w-3xl items-center gap-2 px-5 py-3">
          <button onClick={() => setOpenId(null)} className="rounded-lg p-2" title="Csoportjaim">
      ...
      <main className="mx-auto max-w-3xl px-5 py-6">
        ...
        {tab === "tracker" && <TrackerView me={me} group={group} sports={sports}
        ...
        {tab === "wallet" && <WalletView me={me} group={group} stats={stats} pool={pool}
          onDeclare={declare} onApprove={approve} onInvite={inviteEmail} toast={notify} />}
      </main>
