...
import {
  ...
  MessageSquare, Scale, Route, Timer, Beer, Medal
} from "lucide-react";
...
const Logo = ({ size = "sm" }) => {
  ...
  return (
    <div className="flex items-center gap-2">
      <svg width={big ? 26 : 20} height={big ? 34 : 26} viewBox="0 0 26 34">
        <defs>
          <linearGradient id="logoLiquid" x1="0" y1="0" x2="0" y2="1">
            <stop offset="0%" stopColor={C.cyan} /><stop offset="100%" stopColor={C.blue} />
          </linearGradient>
        </defs>
        <rect x="9" y="0" width="8" height="4" rx="1.5" fill={C.blueDeep} />
        <path d="M9 5 h8 v2 c0 1.5 1 2 2 3 c1.8 1.8 2.4 3.6 2.4 6 v13 c0 2.4 -1.6 4 -4 4 h-8.8 c-2.4 0 -4 -1.6 -4 -4 v-13 c0 -2.4 .6 -4.2 2.4 -6 c1 -1 2 -1.5 2 -3 z"
          fill="url(#logoLiquid)" />
      </svg>
      <div className="leading-none">
        <p className={`font-black tracking-tight ${big ? "text-base" : "text-[13px]"}`} style={{ color: C.ink }}>
          POWERADE
        </p>
        <p className={`font-black tracking-tight ${big ? "text-base" : "text-[13px]"}`} style={{ color: C.blue }}>
          CHALLENGE
        </p>
      </div>
    </div>
...
function PoweradeBottle({ pct, daysLeft, beforeStart }) {
  ...
  return (
    <div className="relative flex flex-col items-center">
      ...
      <svg width="150" height="280" viewBox="0 0 150 280" onClick={pop}
        role="button" tabIndex={0} onKeyDown={(e) => e.key === "Enter" && pop()}
          ...
          <div className="space-y-2">
            {mine.map((w) => (
              <div key={w.id} className="flex items-center gap-3 rounded-xl px-3 py-3" style={{ border: `1px solid ${C.line}` }}>
                ...
                {w.photo_url && (
                  <a href={w.photo_url} target="_blank" rel="noreferrer" className="text-xs font-semibold" style={{ color: C.slate }}>
                    <Camera size={13} />
                  </a>
                )}
...
function LeaderboardView({ group, meId, onOpenMember }) {
  ...
  const rows = useMemo(() => group.members.map((u) => {
    ...
    return {
      id: u.id, name: u.name, emoji: u.emoji, gender: u.gender, goal: s.goal, target, kcal,
    ...
    <div className="space-y-5">
      ...
      <Card className="overflow-hidden">
        <div className="flex items-center gap-3 px-4 py-3 text-xs font-bold uppercase tracking-wider"
          ...
          {pen && <span className="w-24 text-right">Tartozás</span>}
        </div>
        {rows.map((r, i) => (
          <button key={r.id} onClick={() => onOpenMember(r.id)}
            ...
            <div className="min-w-0 flex-1">
              ...
              <div className="mt-1.5 flex items-center gap-2">
                ...
                <span className="shrink-0 text-xs font-bold tabular-nums" style={{ color: r.done ? C.ok : C.slate }}>
            ...
            {pen && (
              <span className="w-24 shrink-0 self-start pt-0.5 text-right text-sm font-bold tabular-nums"
                style={{ color: r.debt ? C.danger : C.ok }}>
                {r.debt ? money(r.debt) : "—"}
              </span>
            )}
...
export default function App() {
  ...
  const load = useCallback(async () => {
    if (!session) return;
    const [pr, en, pa, gr, gm, inv, we, be, ba] = await Promise.all([
    ...
    const bad = [pr, en, pa, gr, gm, inv, we, be, ba].find((r) => r.error);
    ...
    const users = (pr.data || []).map((p) => {
      ...
      const beers = {};
      (be.data || []).filter((x) => x.user_id === p.id).forEach((x) => { beers[x.date] = x.count; });
    ...
    <div className="min-h-screen pb-24 md:pb-8" style={{ background: "#fff" }}>
      <header className="sticky top-0 z-20 backdrop-blur"
        ...
        <div className="mx-auto flex max-w-3xl items-center gap-2 px-4 py-2.5">
          ...
          <Logo />
          <div className="mx-1 hidden h-8 w-px sm:block" style={{ background: C.line }} />
          <div className="hidden min-w-0 flex-1 sm:block">
            <p className="truncate text-xs font-black" style={{ color: C.ink }}>{group.name}</p>
            <p className="text-xs font-bold" style={{ color: C.blue }}>{goalOf(me).toLocaleString("hu-HU")} kcal / hét</p>
          </div>
