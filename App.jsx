...
import {
  ...
  ChevronRight, LogOut, AlertTriangle, Clock, Camera, X, ShieldCheck, RefreshCw
} from "lucide-react";

/* ============================================================================
   POWERADE CHALLENGE  ·  Supabase-verzió
   ----------------------------------------------------------------------------
   Szabályok:
   ...
   FONTOS: bejegyzést csak a mai vagy a tegnapi napra lehet rögzíteni.
   Ezt maga az adatbázis kényszeríti ki, tehát a múltat senki nem írhatja át.
   ========================================================================== */
...
const CHALLENGE_START = "2026-07-13"; // hétfő – a kihívás első napja
...
function evaluateWeek(entries, weekIndex) {
  ...
  const missedDay = days.some((d) => d >= CHALLENGE_START && d < TODAY && !(entries[d]?.kcal > 0));

  let status = "upcoming";
  if (started) {
    if (missedDay) status = "failed";
...
function evaluateUser(user) {
  ...
  return { weeks, penalties, paid, pending, debt, streak, totalKcal };
}
    ...
    <div className="min-h-screen px-5 py-10" style={{ background: "#fff" }}>
      <div className="mx-auto flex w-full max-w-md flex-col">
        ...
        <div className="space-y-4">
          {mode === "signup" && (
            <Field label="Név" value={name} onChange={(e) => setName(e.target.value)} placeholder="Kis Márton" />
          )}
          <Field label="E-mail" type="email" value={email} onChange={(e) => setEmail(e.target.value)} placeholder="te@pelda.hu" />
          <Field label="Jelszó" type="password" value={pw} onChange={(e) => setPw(e.target.value)} placeholder="legalább 6 karakter"
          ...
          <Button onClick={submit} disabled={busy} className="w-full py-4 text-base">
            {busy ? "Egy pillanat…" : mode === "signup" ? "Belépek a kihívásba" : "Belépés"}
  ...
  return (
    <div className="relative flex items-center justify-center">
      ...
      <div className="absolute flex flex-col items-center">
        ...
        <span className="mt-1 text-xs font-bold" style={{ color: pct >= 1 ? C.ok : C.blue }}>{Math.round(pct * 100)}%</span>
      </div>
    </div>
  );
}
...
function DashboardView({ me, stats, pool, go }) {
  ...
  return (
    <div className="space-y-5">
      <Card className="p-6">
        <SectionTitle right={
          <Pill tone={week.missedDay ? "danger" : week.total >= WEEKLY_GOAL ? "ok" : "blue"}>
        ...
        {!todayEntry && (
          <div className="mt-5 flex items-center gap-3 rounded-xl p-4" style={{ background: `${C.amber}12` }}>
            ...
            <p className="flex-1 text-sm font-semibold" style={{ color: C.ink }}>
              Ma még nincs bevitt mozgás. Éjfélig pótold, különben bukik a hét.
            </p>
      ...
      <Card className="p-6" style={{ background: `linear-gradient(135deg, ${C.blueDeep}, ${C.blue})`, border: "none" }}>
        <div className="flex items-start justify-between">
          <div>
            ...
            <p className="mt-1 text-sm font-medium text-white opacity-80">Befizetve a csapat által</p>
          </div>
        ...
        <div className="space-y-4">
          <div>
            ...
            <p className="mt-2 text-xs" style={{ color: C.slate }}>
              Régebbi napra nem lehet rögzíteni — a múlt le van zárva.
            </p>
          </div>
            ...
            return (
              <button key={d} onClick={() => editable && setDate(d)} disabled={!editable}
                ...
                style={{ background: d === date ? `${C.blue}0D` : "transparent",
                  border: `1px solid ${d === date ? C.blue : C.line}`, opacity: editable || e ? 1 : 0.6 }}>
...
function LeaderboardView({ users, meId }) {
  ...
  }).sort((a, b) => b.kcal - a.kcal), [users, tab, cw]);
  ...
  return (
    <div className="space-y-5">
      ...
      <p className="px-1 text-xs" style={{ color: C.slate }}>
        A „Befizetve" oszlop a kasszába már beérkezett, jóváhagyott büntetéseket mutatja.
      </p>
    </div>
  );
}
...
/* ============================== Pénztárca ================================= */
function WalletView({ me, users, stats, pool, onDeclare, onApprove, toast }) {
  const failed = stats.weeks.filter((w) => w.status === "failed");
  const pendingAll = users.flatMap((u) => u.payments.filter((p) => p.status === "pending").map((p) => ({ ...p, user: u })));

  const declare = async () => {
    if (stats.debt <= 0) return toast("Nincs rendezetlen büntetésed.", "ok");
    const ok = await onDeclare(stats.debt);
    if (ok) toast("Befizetés igazolva — admin jóváhagyásra vár.", "ok");
  };
  ...
  return (
    <div className="space-y-5">
      ...
      {me.is_admin && (
        <Card className="p-6">
          <SectionTitle right={<Pill tone="blue"><ShieldCheck size={12} /> Admin</Pill>}>Jóváhagyásra vár</SectionTitle>
          {pendingAll.length === 0 ? (
            ...
            <div className="space-y-2">
              {pendingAll.map((p) => (
                <div key={p.id} className="flex items-center gap-3 rounded-xl px-3 py-3" style={{ border: `1px solid ${C.line}` }}>
                  ...
                  <Button onClick={() => onApprove(p.id)} className="px-3 py-2">
                    <Check size={14} /> Megérkezett
                  </Button>
                </div>
              ))}
            </div>
...
export default function App() {
  ...
  const load = useCallback(async () => {
    ...
    if (p.error || e.error || pay.error) {
      notify("Nem sikerült betölteni az adatokat.", "danger");
      return;
    }
    const built = (p.data || []).map((prof) => ({
      ...prof,
      entries: Object.fromEntries((e.data || []).filter((x) => x.user_id === prof.id).map((x) => [x.date, x])),
      payments: (pay.data || []).filter((x) => x.user_id === prof.id),
    }));
    setUsers(built);
    const custom = [...new Set((e.data || []).map((x) => x.sport))].filter((s) => !DEFAULT_SPORTS.includes(s));
  ...
  const pool = useMemo(() => users.reduce((s, u) =>
    s + u.payments.filter((p) => p.status === "paid").reduce((x, p) => x + p.amount, 0), 0), [users]);
  ...
  const declare = async (amount) => {
    const { error } = await supabase.from("payments").insert({ user_id: session.user.id, amount });
    if (error) { notify("Nem sikerült rögzíteni: " + error.message, "danger"); return false; }
    await load();
    return true;
  };
  ...
  if (!me) return (
    <div className="flex min-h-screen flex-col items-center justify-center gap-4" style={{ background: "#fff" }}>
      <RefreshCw className="animate-spin" style={{ color: C.blue }} />
      <Button variant="ghost" onClick={() => supabase.auth.signOut()}>Kilépés</Button>
    </div>
  );
    ...
    <div className="min-h-screen pb-24 md:pb-8" style={{ background: "#fff" }}>
      <header className="sticky top-0 z-20 backdrop-blur"
        ...
        <div className="mx-auto flex max-w-3xl items-center gap-3 px-5 py-3">
          ...
          <button onClick={() => supabase.auth.signOut()} className="rounded-lg p-2">
      ...
      <main className="mx-auto max-w-3xl px-5 py-6">
        ...
        {tab === "leaderboard" && <LeaderboardView users={users} meId={me.id} />}
        {tab === "wallet" && <WalletView me={me} users={users} stats={stats} pool={pool} onDeclare={declare} onApprove={approve} toast={notify} />}
      </main>
