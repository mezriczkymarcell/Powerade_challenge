...
import {
  ...
  MessageSquare, Scale, Route, Timer, Beer, Medal, Heart
} from "lucide-react";
...
function Feed({ group, meId, likes, onLike }) {
  const items = useMemo(() => {
    ...
    group.members.forEach((u) => {
      Object.values(u.entries).forEach((d) => d.items.forEach((it) => out.push({
      ...
      (u.beerRows || []).forEach((b) => { if (b.count > 0) out.push({
  ...
  return (
    <Card className="p-6">
      <SectionTitle right={<Pill tone="blue">{group.members.length} tag</Pill>}>Mi történik a csapatban</SectionTitle>
      <div>
        {items.map((it) => (
